...
## ROI 52 architecture notes

### `MIOOSPERM`

`MIOOSPERM` owns the shell permission matrix. Targets are stored under `^MIO("MIOOS","PERM","TARGET",...)` and currently cover:

- built-in apps such as `app:terminal` and `app:security-center`
- module launcher targets such as `module:module-notes`
- shell administration targets such as permission reporting and module installation

The server remains authoritative for allow/deny decisions. The browser only edits allowed fields through websocket commands.

### `MIOOSMOD`

`MIOOSMOD` manages user-scope and system-scope module manifests under `^MIO("MIOOS","MODULE","MANIFEST",...)`.

Manifest fields are intentionally compact and boot-contract-friendly:

- identity and presentation: `id`, `title`, `subtitle`, `description`, `icon`, `category`, `version`
- shell host contract: `surface`, `windowTitle`, `singleton`, `launcherEnabled`
- content metadata: `cards[]`, `params[]`

### UI kit posture

ROI 52 introduces a small shared UI kit in CSS rather than a separate JS component runtime. Current primitives include:
...
This keeps the shell SSR-first and Vue-thin while reducing visual drift between built-in apps.
