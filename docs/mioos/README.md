...
Markdown files now open in a rendered preview backed by the vendored local Marked UMD file at `public/mioos/vendor/marked/lib/marked.umd.js`; editing continues through CodeMirror for bounded text files. Markdown renders as sanitized inline HTML with shell theme styling and must not use an iframe. HTML previews render inside sandboxed iframes without script execution. Large Markdown/source-edit files stay in the chunked read-only text viewer instead of requesting a full blob. PDF files use the browser-native PDF viewer through the authenticated local file route and keep the normal download fallback.
...
## ROI 105/106 Markdown inline preview and terminal text-load visibility

Markdown is now rendered as sanitized inline HTML inside the MIOOS viewer pane, not in an iframe. Dark theme support is explicit through `.mioos-markdown-inline-preview`, `.mioos-markdown-preview-content`, and `.theme-dark-mode` CSS rules, including backgrounds, text, links, code blocks, blockquotes, and tables. HTML remains the only iframe preview path.

Full text loading now updates visible viewer content as chunks arrive and prevents CodeMirror from mounting until the complete editable content is loaded. Terminal chunk/load errors set a stop flag so lifecycle hooks do not re-enter the same failed load automatically; refresh or one bounded manual retry is required.
