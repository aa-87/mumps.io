...
## ROI 104 text chunk load session loop guard

Text file opening now uses a deterministic load-session contract for `/api/mioos/fs/text-chunk`. Each session tracks `sessionId`, file id, mode (`preview` or `full-edit`), status, chunk size, requested keys, completed offsets, failed offsets, seen offsets, request count, max requests, last/next offset, EOF, expected bytes, loaded bytes, and the last error. The frontend request key is exactly `fileId:offset:size`, and an in-flight key cannot be issued again.

Full-edit loading follows backend `nextOffset` exactly. It does not align `nextOffset` backward to a chunk boundary. Loading stops with visible UI feedback if the backend returns an already seen offset, a non-increasing `nextOffset`, missing/unsafe progress metadata, zero bytes without EOF, malformed metadata, the browser editable-byte cap, or the hard max chunk request count. Large or unknown-size text still opens as one bounded preview chunk and requires the explicit **Edit File as Text** action before any guarded full-load attempt.

The HTTP and WebSocket text chunk backends now expose byte-oriented metadata (`requestedSize`, `bytes`, `readBytes`, `nextOffset`, `eof`, `size`) from `READWIN^MIOOSFS`. Empty non-EOF text chunks are treated as errors instead of being allowed to drive another identical request. Markdown preview still uses the local Marked renderer, and its sandboxed iframe receives light/dark theme colors from the active shell theme. HTML preview remains a direct sandboxed iframe backed by the authenticated blob route and does not call `/api/mioos/fs/text-chunk` unless the user explicitly chooses source editing.
