...
- The shell chrome now uses a more professional taskbar, start menu, and desktop context menu layout oriented around production desktop workflows.
- Security Center now includes launcher/module permission governance in addition to auth, sessions, lockouts, and audit visibility.
- App Catalog now acts as a Module Studio with documented manifest fields, install scopes, and removal of custom module manifests.
- A shared MIOOS UI kit now backs cards, tables, toolbars, stat grids, and form layouts so built-ins and future apps can converge visually.
- Dark-theme contrast and control readability were hardened across window surfaces, menus, tables, buttons, forms, and popup shells.
