...
MIOOS now includes a HIPAA-aligned permissions administration subsystem for deployments that enable internal/user modules.

Backend routine: `MIOOSPERM`

Security model:

- permissions are explicit records;
- permission groups collect permissions;
- permission profiles collect groups;
- profiles are assigned to users or roles;
- PHI-related permissions are flagged;
- sensitive/break-glass permissions are flagged;
- administrative changes are written to a non-PHI audit trail;
- module UI traffic uses WebSocket commands only.

Reference permission examples:

- `mioos.permissions.view`
- `mioos.permissions.edit`
- `mioos.permissions.admin`
- `mioos.permissions.audit`
- `mioos.modules.run`
- `mioos.table.query`
- `mioos.phi.read`
- `mioos.phi.write`
- `mioos.phi.export`
- `mioos.phi.breakglass`

The default profiles include `hipaa-admin`, `minimum-necessary`, and `audit-review`. These provide a deployable baseline for least privilege and minimum necessary access, but they must be reviewed against the organization's policies before production use.

This is a technical control foundation. A system is not automatically HIPAA compliant simply because this module is present; compliance also depends on infrastructure, encryption, access procedures, training, BAAs, incident response, and retention policy.
