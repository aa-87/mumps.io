...
## Table-backed module registry contract (ROI 91)

User-created table modules generated by UI Modules are registered through `SAVE^MIOOSMTBL` and `INSTALL^MIOOSMTBL`. During validation the backend now normalizes table module definitions so malformed frontend-supplied component or surface values cannot create broken launch entries:
