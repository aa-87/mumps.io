...
## ROI 83 — uploaded desktop background asset streaming

Uploaded desktop backgrounds can travel through two backend-authored asset paths:

- `/api/mioos/theme-asset?id=...` for active theme assets and login-safe public assets.
- `/api/mioos/fs/blob?id=...` for promoted desktop wallpaper files copied into VFS by `PROMOTEW^MIOOSTHEME`.

Both paths must be safe for binary image data. Dynamic theme assets and promoted wallpaper blobs are streamed with HTTP chunked transfer framing instead of a fixed `Content-Length`. This prevents browser `ERR_CONTENT_LENGTH_MISMATCH` when a MUMPS runtime stores image chunks with binary-safe `$ZLENGTH` semantics but the socket write path normalizes bytes differently.

Regression coverage in `T083^MIOOST` locks these requirements:

- promoted wallpaper VFS blobs are detected by `themeWallpaper` / `sourceAsset` metadata;
- uploaded theme assets use binary-safe chunked transfer framing;
- chunk sizes are derived from `$ZLENGTH`, not character length;
- chunked responses are terminated with a zero-length chunk;
- `Content-Length` is removed from dynamic uploaded-background image responses.
