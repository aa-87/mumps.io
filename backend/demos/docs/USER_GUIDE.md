...
MIOTPL is a Mustache-first template engine for MUMPS (YottaDB/GT.M) used by MUMPS.IO. It supports core Mustache, plus a small set of practical extensions:
...
S CTX("name")="Ahmed"
D RENDER^MIOTPL("pages/home",.CONF,.CTX,.OUT,.ERR)
...
D RENDERANY^MIOTPL("Hello {{name}}!",.CONF,.CTX,.OUT,.ERR)
I $D(ERR) ZWRITE ERR Q
W OUT
```
...
D RENDERPAGE^MIOTPL("pages/about","layouts/main",.CONF,.CTX,.OUT,.ERR)
I $D(ERR) ZWRITE ERR Q
W OUT
```
