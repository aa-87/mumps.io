...
ENUM1(PAT,LIST) ;
	NEW F
	FOR  SET F=$ZSEARCH(PAT)  QUIT:F=""  DO
	. SET LIST(F)=1
	QUIT
	;
...
TRIM(S) ;
	...
	NEW X SET X=$GET(S)
	FOR  QUIT:X=""  QUIT:($E(X,1)'=" ")&($E(X,1)'=$C(9))  SET X=$E(X,2,$L(X))
	FOR  QUIT:X=""  QUIT:($E(X,$L(X))'=" ")&($E(X,$L(X))'=$C(9))  SET X=$E(X,1,$L(X)-1)
	QUIT X
	;
ESC(S) ;
