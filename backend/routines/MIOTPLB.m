...
BENCHMIN(CONF,TPL,CTX,MSCOM,MSREN,OUTSZ,ERR) ;
	...
	. NEW TOK KILL TOK
	. DO COMPILE^MIOTPL2($G(TPL),.TOK,.ERR)
	SET TE=$$NOWUS() QUIT:$DATA(ERR)
	SET EL=TE-TS
	SET MSCOM=$$MSPER(EL,CI)
	NEW TOK KILL TOK
	DO COMPILE^MIOTPL2($G(TPL),.TOK,.ERR) QUIT:$DATA(ERR)
	...
	NEW OUT DO EVAL^MIOTPL2(.TOK,.CONF,.CTX,.OUT,.ERR) QUIT:$DATA(ERR)
	SET OUTSZ=$L($G(OUT))
	QUIT
	;
BENCHPERF(CONF,NAME,TPL,CTX,MSCOM,MSREN,OUTSZ,ERR) ;
	...
	IF WARM DO  QUIT:$DATA(ERR)
	. NEW OUT
	. DO EVAL^MIOTPL2(.HTOK,.CONF,.CTX,.OUT,.ERR)
	SET TS=$$NOWUS()
	F I=1:1:RI DO  QUIT:$DATA(ERR)
	. NEW OUT
	. DO EVAL^MIOTPL2(.HTOK,.CONF,.CTX,.OUT,.ERR)
	...
	SET EL=TE-TS
	SET MSREN=$$MSPER(EL,RI)
	NEW OUT DO EVAL^MIOTPL2(.HTOK,.CONF,.CTX,.OUT,.ERR) QUIT:$DATA(ERR)
	SET OUTSZ=$L($G(OUT))
	QUIT
	;
BENCHBIG(CONF,NAME,TPL,CTX,MSCOM,MSREN,OUTSZ,ERR) ;
	...
	IF WARM DO  QUIT:$DATA(ERR)
	. NEW DUM
	. DO EVALX^MIOTPL2(.HTOK,.CONF,.CTX,"R",.DUM,OREF,.ERR)
	SET TS=$$NOWUS()
	F I=1:1:RI DO  QUIT:$DATA(ERR)
	. NEW DUM
	. DO EVALX^MIOTPL2(.HTOK,.CONF,.CTX,"R",.DUM,OREF,.ERR)
	...
	SET MSREN=$$MSPER(EL,RI)
	NEW DUM
	DO EVALX^MIOTPL2(.HTOK,.CONF,.CTX,"R",.DUM,OREF,.ERR) QUIT:$DATA(ERR)
...
VER() ;
	...
	IF $T(VERSION^MIOTPL2)'="" SET V=$$VERSION^MIOTPL2()
