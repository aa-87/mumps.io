...
EN ;
	...
	;
	D T("SHA256 1000 a",$$SHA256^MIOSHA256($$REP("a",1000)),"41edece42d63e8d9bf515a9ba6932e1c20cbc9f5a5d134645adb5db1b9737ea3",.TOTAL,.FAIL)
	D TV1(.TOTAL,.FAIL)
	D TV2(.TOTAL,.FAIL)
	D TV3(.TOTAL,.FAIL)
	D TV4(.TOTAL,.FAIL)
	...
	I FAIL  W "MIOSHA256 TEST FAILURES DETECTED",!
	Q
	;
TV1(TOTAL,FAIL) ;
	N KEY,DATA,EXP,ACT
	...
	S ACT=$$RAW2HEX^MIOSHA256($$HMACRAW^MIOSHA256(KEY,DATA))
	D T("HMACRAW RFC4231 #1",ACT,EXP,.TOTAL,.FAIL)
	Q
	;
TV2(TOTAL,FAIL) ;
	N KEY,DATA,EXP,ACT
	...
	S ACT=$$RAW2HEX^MIOSHA256($$HMACRAW^MIOSHA256(KEY,DATA))
	D T("HMACRAW RFC4231 #2",ACT,EXP,.TOTAL,.FAIL)
	Q
	;
TV3(TOTAL,FAIL) ;
	...
	;
TV4(TOTAL,FAIL) ;
	N REF,ROOT,EXP,ACT,RAW
	K ^TMP($J,"MIOSHA256T")
	S ROOT=$NAME(^TMP($J,"MIOSHA256T","ref"))
	S @ROOT@(1)="alpha "
	S @ROOT@(2)="beta"
	S @ROOT@(3)=" gamma"
	S EXP=$$SHA256^MIOSHA256("alpha beta gamma")
	S ACT=$$SHA256REF^MIOSHA256(ROOT)
	D T("SHA256REF chunked",ACT,EXP,.TOTAL,.FAIL)
	S RAW=$$RAW2HEX^MIOSHA256($$SHA256REFRAW^MIOSHA256(ROOT))
	D T("SHA256REFRAW chunked",RAW,EXP,.TOTAL,.FAIL)
	K ^TMP($J,"MIOSHA256T")
	Q
	;
