...
MESSAGE(DEV,CONF,REQ,CTX)
	...
	IF 'OK DO  QUIT
	. IF $$SIGNOUTCODE($GET(ERR("error"))) DO  IF 1
	...
	. DO SENDTEXT^MIOWS(.DEV,RESP)
	. MERGE CSTATE=STATE
	. IF $$CANUSE^MIOMOSCHAT(.CSTATE,$GET(CSTATE("chatRoom"))) DO
	. . DO MARKREAD^MIOMOSCHAT(.CSTATE,$GET(CSTATE("chatRoom")))
	. . DO SNAPSHOT^MIOMOSCHAT($GET(CSTATE("chatRoom")),+$GET(CSTATE("chatLimit"),20),.RESP)
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	. . DO METAJSON^MIOMOSCHAT(.CSTATE,.RESP)
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	IF EVT="ping" DO  QUIT
	. NEW CONNID
	. SET CONNID=$$CONNID(PAYLOAD,.CTX,.STATE)
	...
	. IF '$$CANUSE^MIOMOSCHAT(.STATE,ROOM) DO  QUIT
	. . SET RESP=$$FORBIDDENJSON("chat.room")
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	. MERGE CSTATE=STATE
	. SET CSTATE("chatRoom")=ROOM
	. DO MARKREAD^MIOMOSCHAT(.CSTATE,ROOM)
	. DO SNAPSHOT^MIOMOSCHAT(ROOM,+$GET(CSTATE("chatLimit"),20),.RESP)
	. DO SENDTEXT^MIOWS(.DEV,RESP)
	. DO METAJSON^MIOMOSCHAT(.CSTATE,.RESP)
	. DO SENDTEXT^MIOWS(.DEV,RESP)
	...
	. DO METAJSON^MIOMOSCHAT(.CSTATE,.RESP)
	. DO SENDTEXT^MIOWS(.DEV,RESP)
	IF EVT="chat.delete" DO  QUIT
	. NEW CSTATE,DELID,CHATOUT
	. IF '$$HAS^MIOMOSPERM(.STATE,"chat.moderate") DO  QUIT
	. . SET RESP=$$FORBIDDENJSON("chat.moderate")
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	. SET ROOM=$$ROOM(PAYLOAD,.STATE)
	. IF '$$CANUSE^MIOMOSCHAT(.STATE,ROOM) DO  QUIT
	. . SET RESP=$$FORBIDDENJSON("chat.room")
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	. SET DELID=$$MSGID(PAYLOAD)
	. IF '$$DELETE^MIOMOSCHAT(.STATE,ROOM,DELID,.CHATOUT,.ERR) DO  QUIT
	. . SET RESP=$$ERRJSON("chat_delete_error",$GET(ERR("error")))
	. . DO SENDTEXT^MIOWS(.DEV,RESP)
	. MERGE CSTATE=STATE
	. SET CSTATE("chatRoom")=ROOM
	. DO MARKREAD^MIOMOSCHAT(.CSTATE,ROOM)
...
MSGID(PAYLOAD)
	NEW TREE,ERR
	IF $EXTRACT($GET(PAYLOAD),1)'="{" QUIT 0
	IF '$$DECODE^MIOJSON($GET(PAYLOAD),.TREE,.ERR) QUIT 0
	QUIT +$GET(TREE("id"))
	;
SESSIONID(PAY)
	NEW TREE,ERR,SID
