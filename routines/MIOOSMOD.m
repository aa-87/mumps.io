...
INTERNAL(OUT)
	...
	SET OUT("components",C,"name")="mioos-permissions-panel"
	SET OUT("components",C,"title")="Permissions UI"
	SET OUT("components",C,"surface")="mioos-surface-permissions"
	SET OUT("components",C,"source")="internal"
	SET OUT("components",C,"owner")="MIOOS"
	SET OUT("components",C,"description")="Owner, sharing, and read-only metadata editor backed by MIOOSFS."
	SET OUT("components",C,"script")="/public/mioos/app/mioos_permissions.js"
	SET OUT("components",C,"backend")="MIOOSFS"
	SET OUT("components",C,"features",1)="fs.meta"
	SET OUT("components",C,"features",2)="fs.setmeta"
	SET OUT("components",C,"features",3)="owner-role-flags"
	...
	SET M=+$GET(OUT("moduleCount"))+1,OUT("moduleCount")=M
	SET OUT("modules",M,"id")="mioos.sample.table"
	SET OUT("modules",M,"key")="sample-table"
	SET OUT("modules",M,"appKey")="sample-table"
	SET OUT("modules",M,"title")="Sample Table"
	SET OUT("modules",M,"description")="Launches the backend-driven sample table dataset with paging, sorting, grouping, selection, and actions."
	SET OUT("modules",M,"category")="Examples"
	SET OUT("modules",M,"icon")="▦"
	SET OUT("modules",M,"source")="internal"
	SET OUT("modules",M,"builtIn")=1
	SET OUT("modules",M,"componentKey")="table"
	SET OUT("modules",M,"surface")="mioos-surface-table"
	SET OUT("modules",M,"tableState","id")="mioos-sample-table"
	SET OUT("modules",M,"tableState","title")="Sample Table"
	SET OUT("modules",M,"tableState","dataset")="demo"
	SET M=+$GET(OUT("moduleCount"))+1,OUT("moduleCount")=M
	SET OUT("modules",M,"id")="mioos.permissions"
	SET OUT("modules",M,"key")="permissions"
	SET OUT("modules",M,"appKey")="permissions"
	SET OUT("modules",M,"title")="Permissions UI"
	SET OUT("modules",M,"description")="Inspect and update VFS owner/share/read-only flags through existing fs.meta and fs.setmeta contracts."
	...
	SET OUT("examples",2,"title")="Permissions UI Example"
