...
LOOP(DEV,CONF,REQ,CTX)
	...
	NEW MAXFRAME SET MAXFRAME=+$GET(CONF("websocket","maxFrameBytes"),128000)
	NEW MAXMSG SET MAXMSG=+$GET(CONF("websocket","maxMessageBytes"),128000*4)
...
READFRAME(DEV,TO,FIN,OPC,PAY,ERR,MAXFRAME)
	KILL ERR
	NEW HDR,CBCTX
	DO READFRAMEHDR(.DEV,TO,.HDR,.ERR,+$GET(MAXFRAME,128000)) QUIT:$DATA(ERR)
...
READFRAMEHDR(DEV,TO,HDR,ERR,MAXFRAME)
	...
	IF $DATA(ERR) QUIT
	IF HDR("len")=126 DO
	. SET X="" DO READN^MIOSOCK(DEV,2,TO,.X) IF '$TEST SET ERR("error")="ws_timeout" QUIT
	. SET HDR("len")=($ASCII($EXTRACT(X,1))*256)+$ASCII($EXTRACT(X,2))
	IF HDR("len")=127 DO
	. SET X="" DO READN^MIOSOCK(DEV,8,TO,.X) IF '$TEST SET ERR("error")="ws_timeout" QUIT
	. SET VAL=0 FOR I=1:1:8 SET VAL=VAL*256+$ASCII($EXTRACT(X,I))
	. IF VAL>2147483647 SET ERR("error")="ws_frame_too_large" QUIT
	. SET HDR("len")=VAL
	IF $GET(HDR("len"))>+$GET(MAXFRAME,128000) SET ERR("error")="ws_frame_too_large" QUIT
...
READPAYLOADST(DEV,TO,HDR,ERR,CHUNKBYTES,CALLBACK,CBCTX)
	...
	. SET PART=""
	. DO READN^MIOSOCK(DEV,TAKE,TO,.PART) IF '$TEST SET ERR("error")="ws_timeout" QUIT
...
READMSGST(DEV,S,OPC,ERR,CHUNKBYTES,CALLBACK,CBCTX)
	...
	. DO READFRAMEHDR(.DEV,+$G(S("to"),1),.HDR,.ERR,+$G(S("maxframe"),128000))
...
INITSTATE(S,CONF)
	...
	SET S("maxframe")=+$GET(CONF("websocket","maxFrameBytes"),128000)
