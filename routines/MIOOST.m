...
T042 QUIT
	...
	SET STATE("principal")=$GET(CONF("mioos","bootstrapAuth","admin","username"),"admin")
	SET STATE("roles")=$GET(CONF("mioos","bootstrapAuth","admin","roles"),"admin")
	SET SEG1=$TRANSLATE($JUSTIFY("",860000)," ","A")
	SET SEG2=$TRANSLATE($JUSTIFY("",860000)," ","B")
	...
	SET ID=$GET(OUT("id"))
	DO EQ^MIOTASSERT(+$GET(^MIO("MIOOS","FS","INFO",ID,"chunkSize")),860000,"[MIOOST][T042][stored chunk size]")
...
T047
	...
	DO EQ^MIOTASSERT(+$GET(BOOT("vfs","mediaWarmupBytes")),860000,"[MIOOST][T047][boot media warmup bytes]")
...
T049
	...
	DO EQ^MIOTASSERT(+$GET(BOOT("vfs","mediaWarmupBytes")),860000,"[MIOOST][T049][media warmup bytes]")
	...
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","IF +$GET(ISMEDIA)=1,SENDCH>860000 SET SENDCH=860000"),"[MIOOST][T049][media send cap]")
...
T051
		...
		DO OK^MIOTASSERT($$FILEHAS("templates/layouts/mioos_shell.html","7css-shell-plus-basecoat-augment"),"[MIOOST][T051][basecoat augment layout]")
...
T058
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","vm.explorerPromptUpload(window.id)")!$$FILEHAS("public/mioos/app/mioos_shell_ui.js","explorerRunContextAction(window.id, 'upload')")!$$FILEHAS("public/mioos/app/mioos_explorer.js","explorerPromptUpload(windowId)"),"[MIOOST][T058][upload action retained]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","vm.explorerDownloadSelected(window.id)")!$$FILEHAS("public/mioos/app/mioos_shell_ui.js","explorerRunContextAction(window.id, 'download')")!$$FILEHAS("public/mioos/app/mioos_explorer.js","explorerDownloadSelected(windowId)"),"[MIOOST][T058][download action retained]")
...
T074
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","readTextChunkViaWebSocket"),"[MIOOST][T074][text viewer chunk reader]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","fs.text.chunk"),"[MIOOST][T074][text viewer chunk command]")
...
T075
	...
	;
T076
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSFS.m","REPAIRSIZE(ID)"),"[MIOOST][T076][vfs repair size helper]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSFS.m","DATASIZE(ID)"),"[MIOOST][T076][vfs data byte size helper]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSFS.m","$ZLENGTH($GET(DATA))"),"[MIOOST][T076][multipart byte length]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","SET SIZE=$$REPAIRSIZE^MIOOSFS(RID)"),"[MIOOST][T076][fsblob repaired content length]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","THEMEPUBLICASSET"),"[MIOOST][T076][login public asset route preserved]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textViewerLoadChunk"),"[MIOOST][T076][text viewer load chunk]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textViewerOnScroll"),"[MIOOST][T076][text viewer scroll chunk mapping]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","fs_blob_unavailable"),0,"[MIOOST][T076][no text viewer blob fallback sentinel]")
...
T078
	...
	QUIT
T083
	NEW CONF,STATE,ROOT,ID,A,B,C,FIRST
	DO CONFDEF^MIOOS(.CONF)
	SET CONF("mioos","fs","chunkSize")=2048
	DO INIT^MIOOSFS(.CONF)
	SET STATE("principal")="admin",STATE("roles")="admin",STATE("authenticated")=1,STATE("fsBlobPath")="/api/mioos/fs/blob"
	KILL ^MIO("MIOOS","THEMEASSET","admin","asset-t083")
	SET A=$$TSTR("A",1000),B=$$TSTR("B",1048),C="IJ"
	SET ^MIO("MIOOS","THEMEASSET","admin","asset-t083","META")="image/png^unit.png^2050^wallpaper^0^0"
	SET ^MIO("MIOOS","THEMEASSET","admin","asset-t083","DATA",1)=A
	SET ^MIO("MIOOS","THEMEASSET","admin","asset-t083","DATA",2)=B
	SET ^MIO("MIOOS","THEMEASSET","admin","asset-t083","DATA",3)=C
	SET ROOT=$NAME(^MIO("MIOOS","THEME","PROFILE","admin","t083-wall"))
	KILL @ROOT
	SET @ROOT@("desktop","wallpaperAssetId")="asset-t083"
	SET ID=$$PROMOTEW^MIOOSTHEME(.STATE,.CONF,"admin","t083-wall",ROOT,1)
	DO OK^MIOTASSERT(ID'="","[MIOOST][T083][promote uneven asset chunks]")
	SET FIRST=$GET(^MIO("MIOOS","FS","DATA",ID,1))
	DO EQ^MIOTASSERT($ZLENGTH(FIRST),2048,"[MIOOST][T083][promote first chunk size]")
	DO EQ^MIOTASSERT($EXTRACT(FIRST,1,5),"AAAAA","[MIOOST][T083][promote first chunk prefix]")
	DO EQ^MIOTASSERT($EXTRACT(FIRST,1000,1000),"A","[MIOOST][T083][promote first chunk boundary A]")
	DO EQ^MIOTASSERT($EXTRACT(FIRST,1001,1001),"B","[MIOOST][T083][promote first chunk boundary B]")
	DO EQ^MIOTASSERT($EXTRACT(FIRST,2048,2048),"B","[MIOOST][T083][promote first chunk suffix]")
	DO EQ^MIOTASSERT($GET(^MIO("MIOOS","FS","DATA",ID,2)),"IJ","[MIOOST][T083][promote final chunk bytes]")
	DO EQ^MIOTASSERT($$DATASIZE^MIOOSFS(ID),2050,"[MIOOST][T083][promote datasize preserved]")
	DO OK^MIOTASSERT($GET(@ROOT@("desktop","wallpaperUrl"))["&v=","[MIOOST][T083][promoted wallpaper cache bust url]")
	...
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","SET SEGPOS=0"),"[MIOOST][T083][wallpaper sender uses cumulative chunk offsets]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","fs_wallpaper_short_write"),"[MIOOST][T083][wallpaper sender detects short writes]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","SET SIZE=SIZE+$ZLENGTH(CH),BUF=BUF_CH"),"[MIOOST][T083][wallpaper promotion rechunks asset data]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","SET ^MIO(""MIOOS"",""FS"",""DATA"",ID,SEG)=$ZEXTRACT(BUF,1,CHUNK)"),"[MIOOST][T083][promoted wallpaper stored at fs chunk size]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","private, no-store, max-age=0"),"[MIOOST][T083][wallpaper blobs avoid stale cache]")
	...
	;
T084
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","readTextChunkViaWebSocket"),"[MIOOST][T084][text chunk reader retained]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textChunkRequestKey"),"[MIOOST][T084][text chunk request key]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","_mioosTextChunkRequests"),"[MIOOST][T084][text chunk inflight dedupe]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","_mioosTextChunkCache"),"[MIOOST][T084][text chunk cache]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","pruneTextViewerChunkCache"),"[MIOOST][T084][text viewer bounded cache]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","openTextViewerWindow"),"[MIOOST][T084][text viewer window]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","openStructuredViewerWindow"),"[MIOOST][T084][structured viewer window]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textStream"),"[MIOOST][T084][structured text stream]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSWS.m","fs.text.chunk"),"[MIOOST][T084][backend text chunk command]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSWS.m","FSTEXTCHUNK"),"[MIOOST][T084][backend text chunk handler]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSWS.m","fs.text.save"),"[MIOOST][T084][backend text save command]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","/api/mioos/fs/text-save")&$$FILEHAS("public/mioos/app/mioos_explorer.js","saveTextViaChunkedHttp"),"[MIOOST][T084][frontend text save route]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","fetchTextBlob"),0,"[MIOOST][T084][no fetchTextBlob regression]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","readTextFileResilient"),0,"[MIOOST][T084][no resilient blob fallback regression]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","fs_blob_unavailable"),0,"[MIOOST][T084][no fs blob fallback sentinel regression]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/README.md","Text viewer regression contract"),"[MIOOST][T084][text viewer docs]")
	DO OK^MIOTASSERT($$FILEHAS("mioos_llm.md","Text media viewer contract"),"[MIOOST][T084][text viewer llm contract]")
	QUIT
	;
TSTR(CH,N)
	NEW OUT,I
	SET OUT="",CH=$GET(CH),N=+$GET(N)
	IF CH="" SET CH="X"
	FOR I=1:1:N SET OUT=OUT_CH
	QUIT OUT
	;
	;
T085
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOS.m","textChunkThresholdBytes"),"[MIOOST][T085][text threshold default config]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textViewerZoom"),"[MIOOST][T085][text zoom method]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","Edit Text"),"[MIOOST][T085][text edit toolbar action]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","Save Text"),"[MIOOST][T085][text save toolbar action]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","Zoom In"),"[MIOOST][T085][text zoom toolbar action]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","mioos-text-editorbar"),0,"[MIOOST][T085][no special text edit div]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_core.js","hideToolbar: true"),"[MIOOST][T085][about hides toolbar state]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","v-if=""!window.hideToolbar"""),"[MIOOST][T085][about toolbar hidden in frame]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","dismissMenus"),"[MIOOST][T085][toolbar click dismiss handler]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","is-click-dismissed"),"[MIOOST][T085][toolbar submenu dismissal css]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","min-width: 34px"),"[MIOOST][T085][larger explorer toolbar icons]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","max-width: calc(100% - 32px)"),"[MIOOST][T085][details filename truncation]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_table.js","mioos-table-patient-banner"),0,"[MIOOST][T085][patient registration banner removed]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","theme-dark-mode .mioos-classic-transferstack"),"[MIOOST][T085][dark transfer background]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","scrollbar-color: #374151 #000000"),"[MIOOST][T085][dark scrollbar colors]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/README.md","Text viewer toolbar and threshold contract"),"[MIOOST][T085][docs text viewer toolbar threshold]")
...
T088
	...
	DO OK^MIOTASSERT($$FSTEXTSAVE^MIOOSWS(.STATE,.CONF,.TREE,.JSON,.ERR),"[MIOOST][T088][text save ignores removed edit cap]")
	DO OK^MIOTASSERT(JSON["fullEditOnDemand","[MIOOST][T088][unbounded full edit marker]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textViewerDraftByteLength"),0,"[MIOOST][T088][no client edit byte cap]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","Edited text is above the bounded-edit save limit"),0,"[MIOOST][T088][no bounded save message]")
...
T090
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","saveTextViaHttp"),"[MIOOST][T090][http text save avoids socket timeout]")
...
T091
	...
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes")),65536,"[MIOOST][T091][default chunk size safe]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","vfs.textChunkSizeBytes || vfs.textChunkBytes || MIOOS_TEXT_WS_SAFE_CHUNK_BYTES"),"[MIOOST][T091][frontend separate chunk config]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","Math.min(safeMax"),"[MIOOST][T091][frontend maxstring safe clamp]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","loadStrategy: virtualized ? 'single-preview-too-large' : 'explicit-complete-http'"),"[MIOOST][T091][large file explicit load mode]")
...
T092
	...
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes")),65536,"[MIOOST][T092][default chunk size safe]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","scrollLoadDisabled: true"),"[MIOOST][T092][scroll load disabled for text]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","retryPolicy: 'bounded-manual-only'"),"[MIOOST][T092][failed chunks are bounded manual retry only]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","explicit-complete-http"),"[MIOOST][T092][large text explicit load mode]")
...
T093
	...
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes")),65536,"[MIOOST][T093][default transport chunk safe]")
	...
	DO OK^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes"))'>860000,"[MIOOST][T093][chunk maxstring safe]")
	...
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@wheel.passive=""armTextScroll"),0,"[MIOOST][T093][wheel does not trigger text load]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@pointerdown=""armTextScroll"),0,"[MIOOST][T093][pointer does not trigger text load]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@pointermove=""armTextScroll"),0,"[MIOOST][T093][pointer drag does not trigger text load]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@touchstart.passive=""armTextScroll"),0,"[MIOOST][T093][touch does not trigger text load]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@touchmove.passive=""armTextScroll"),0,"[MIOOST][T093][touch drag does not trigger text load]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","@keydown=""armTextScroll"),0,"[MIOOST][T093][keyboard does not trigger text load]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-text-scroll-load-disabled"),"[MIOOST][T093][scroll load disabled marker]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","retryPolicy: 'bounded-manual-only'"),"[MIOOST][T093][bounded manual retry policy]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","text_file_too_large_for_browser_editor")&$$FILEHAS("public/mioos/app/mioos_explorer.js","explicit-complete-http"),"[MIOOST][T093][large edit explicit bounded]")
	...
	DO OK^MIOTASSERT($$FSTEXTSAVE^MIOOSWS(.STATE,.CONF,.TREE,.JSON,.ERR),"[MIOOST][T093][large save accepted after cap removal]")
	DO OK^MIOTASSERT(JSON["fullEditOnDemand","[MIOOST][T093][small websocket save compatibility marker]")
...
T097
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","PDF preview")!$$FILEHAS("public/mioos/app/mioos_explorer.js","openPdfViewerWindow"),"[MIOOST][T097][pdf fallback]")
	...
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSFSUP.m","IF N>860000 SET N=860000"),"[MIOOST][T097][upload maxstring clamp]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOS.m","upload"",""chunkBytes"")=860000"),"[MIOOST][T097][upload chunk default separate]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOS.m","textChunkBytes"")=65536"),"[MIOOST][T097][text chunk default safe]")
	...
	;
T098
	NEW CONF,OLD131,OLD262
	SET OLD131=130000+1072,OLD262=260000+2144
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes")),65536,"[MIOOST][T098][text default chunk size]")
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","upload","chunkBytes")),860000,"[MIOOST][T098][upload default chunk size]")
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","download","httpChunkBytes")),860000,"[MIOOST][T098][http default chunk size separate]")
	DO EQ^MIOTASSERT($GET(CONF("mioos","fs","maxTextEditBytes")),"","[MIOOST][T098][max edit default removed]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","avatarUrl")&$$FILEHAS("routines/MIOOSTHEME.m","PUBLICASSET"),"[MIOOST][T098][login avatar persisted marker]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","specificProfile")&$$FILEHAS("routines/MIOOSTHEME.m","LOGINUSER"),"[MIOOST][T098][username-stage specific profile]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","$$SAFE(ID)=ID"),"[MIOOST][T098][public asset id validation]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSTHEME.m","data:"),"[MIOOST][T098][no dataurl login asset]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_auth.js","appliedSpecificTheme = theme"),"[MIOOST][T098][frontend applies username avatar]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_auth.js","appliedSpecificTheme = null"),"[MIOOST][T098][stale username avatar cleared]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-login-avatar-stage"),"[MIOOST][T098][avatar dom marker]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-login-runtime-theme"),"[MIOOST][T098][common background before username]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","genericFeedback"),"[MIOOST][T098][generic username feedback]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_core.js","/api/mioos/theme-asset?id="),0,"[MIOOST][T098][no protected theme asset emitted]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","color: #004cff !important"),"[MIOOST][T098][dark theme editor clear blue]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","data-theme-editor-secondary"),"[MIOOST][T098][dark theme editor selector]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","focus-visible"),"[MIOOST][T098][focus visible preserved]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","mioos-full-table.is-readonly"),"[MIOOST][T098][dark readonly table selector]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","background: #0f172a !important")&$$FILEHAS("public/mioos/mioos.css","color: #f8fafc !important"),"[MIOOST][T098][dark readonly table colors]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css",".mioos-full-table {"),"[MIOOST][T098][advanced table css intact]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-markdown-full-pane-preview"),"[MIOOST][T098][markdown full pane]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-markdown-resize-frame"),"[MIOOST][T098][markdown resize marker]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","Zoom In")&$$FILEHAS("public/mioos/app/mioos_shell_ui.js","Zoom Out"),"[MIOOST][T098][markdown zoom controls]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-mioos-markdown-preview-zoom"),"[MIOOST][T098][markdown zoom srcdoc]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css","data-markdown-full-pane-preview"),"[MIOOST][T098][markdown dark light styles]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","MIOOS_TEXT_STATUS_INFO_TIMEOUT_MS")&$$FILEHAS("public/mioos/app/mioos_explorer.js","statusAutoHideMs"),"[MIOOST][T098][status autohide marker]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","MIOOS_TEXT_STATUS_ERROR_TIMEOUT_MS = 0"),"[MIOOST][T098][error status persistent]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","Show Status")&$$FILEHAS("public/mioos/app/mioos_shell_ui.js","Pin Status"),"[MIOOST][T098][window options status]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-text-status-auto-hide"),"[MIOOST][T098][status does not reserve hidden]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","readTextChunkPreferred"),"[MIOOST][T098][http preferred text chunk]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","MIOOS_TEXT_WS_SAFE_CHUNK_BYTES = 65536"),"[MIOOST][T098][safe websocket chunk ceiling]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","MIOOS_TEXT_VIRTUAL_THRESHOLD_BYTES = 2411725"),"[MIOOST][T098][threshold separate from chunk]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","readTextChunkViaHttp(item, opts)"),"[MIOOST][T098][http first reader]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","textChunkRequestKey(id, offset, size)"),"[MIOOST][T098][chunk dedupe]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","staleTextChunkResponseIgnored"),"[MIOOST][T098][stale chunk ignored]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","full_text_not_loaded_for_save"),"[MIOOST][T098][save waits for complete file]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","FSTEXTCHUNK(DEV,CONF,REQ,CTX)")&$$FILEHAS("routines/MIOOSAPI.m","REQUIREAUTH"),"[MIOOST][T098][http chunk auth]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","IF SIZE>LIMIT SET SIZE=LIMIT"),"[MIOOST][T098][http chunk clamp]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","invalid_offset")&$$FILEHAS("routines/MIOOSAPI.m","invalid_size"),"[MIOOST][T098][invalid chunk request rejected]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","vfs.textChunkSizeBytes || vfs.textChunkBytes || "_OLD131),0,"[MIOOST][T098][no stale frontend chunk fallback]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","Math.min("_OLD262),0,"[MIOOST][T098][no stale frontend clamp]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/README.md","ROI 98"),"[MIOOST][T098][readme docs]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/UI_Modules.md","ROI 98"),"[MIOOST][T098][ui docs]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/Backend_Table.md","ROI 98"),"[MIOOST][T098][backend docs]")
	DO OK^MIOTASSERT($$FILEHAS("mioos_llm.md","ROI 98"),"[MIOOST][T098][llm docs]")
	QUIT

	;
T099
	NEW STATE,CONF,ROOT,OUT,PRE,ERR,USER,AROOT
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	SET STATE("themePublicAssetPath")="/api/mioos/theme-public-asset",STATE("authenticated")=1,STATE("principal")="alice"
	SET ROOT=$NAME(^MIO("MIOOS","THEME","PROFILE","alice","login-theme"))
	SET @ROOT@("key")="login-theme",@ROOT@("id")="login-theme",@ROOT@("name")="Login Theme",@ROOT@("mode")="dark"
	SET @ROOT@("loginScreenConfig","wallpaperUrl")="/api/mioos/theme-asset?id=common-bg"
	SET @ROOT@("loginScreenConfig","avatarUrl")="/api/mioos/theme-asset?id=avatar-img"
	SET @ROOT@("loginScreenConfig","warningImageUrl")="/api/mioos/theme-asset?id=warn-img"
	SET @ROOT@("loginScreenConfig","warningTitle")="Authorized access only"
	SET ^MIO("MIOOS","THEMEASSET","alice","common-bg","META")="image/png^bg.png^1"
	SET ^MIO("MIOOS","THEMEASSET","alice","avatar-img","META")="image/png^avatar.png^1"
	SET ^MIO("MIOOS","THEMEASSET","alice","warn-img","META")="image/png^warning.png^1"
	DO PUBLOGIN^MIOOSTHEME(.STATE,"alice","login-theme",ROOT)
	DO OK^MIOTASSERT($$LOGINPUBLIC^MIOOSTHEME(.CONF,"",.PRE,.ERR),"[MIOOST][T099][common public login theme]")
	DO EQ^MIOTASSERT($GET(PRE("profile","loginScreenConfig","warningImageUrl")),"","[MIOOST][T099][common warning hidden before username]")
	KILL OUT,ERR DO OK^MIOTASSERT($$LOGINPUBLIC^MIOOSTHEME(.CONF,"alice",.OUT,.ERR),"[MIOOST][T099][specific public login theme]")
	DO OK^MIOTASSERT($GET(OUT("specificProfile","loginScreenConfig","warningImageUrl"))["/api/mioos/theme-public-asset?id=warn-img","[MIOOST][T099][specific warning public url]")
	DO EQ^MIOTASSERT($GET(OUT("specificProfile","loginScreenConfig","warningImageAssetId")),"warn-img","[MIOOST][T099][specific warning marker key]")
	DO OK^MIOTASSERT($GET(OUT("specificProfile","loginScreenConfig","warningImageUrl"))'=$GET(OUT("specificProfile","loginScreenConfig","wallpaperUrl")),"[MIOOST][T099][warning distinct from background]")
	DO EQ^MIOTASSERT($GET(OUT("specificProfile","loginScreenConfig","warningImageUrl"))["data:",0,"[MIOOST][T099][warning no dataurl]")
	DO EQ^MIOTASSERT($GET(OUT("specificProfile","loginScreenConfig","warningImageUrl"))["/mnt/",0,"[MIOOST][T099][warning no raw path]")
	DO OK^MIOTASSERT($$PUBLICAS^MIOOSTHEME("warn-img",.USER,.AROOT),"[MIOOST][T099][public warning asset resolves]")
	DO EQ^MIOTASSERT(USER,"alice","[MIOOST][T099][public warning owner scoped]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_auth.js","specificProfile || result.json.profile"),"[MIOOST][T099][frontend prefers specific warning profile]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_auth.js","handleLoginWarningImageError"),"[MIOOST][T099][missing warning clears broken image]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/mioos.css",".theme-dark-mode .mioos-table-showcase .mioos-table-grid tbody td"),"[MIOOST][T099][simple/showcase table dark selector]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","single-preview-too-large")&$$FILEHAS("public/mioos/app/mioos_explorer.js","explicit-complete-http"),"[MIOOST][T099][bounded explicit large text markers]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","text_chunk_loop_guard"),"[MIOOST][T099][large edit loop guard]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","retryPolicy: 'bounded-manual-only'"),"[MIOOST][T099][bounded manual retry only]")
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","textCanEdit: function () { return !!this.textStream; }")&$$FILEHAS("public/mioos/app/mioos_shell_ui.js","data-text-scroll-load-disabled"),"[MIOOST][T099][text edit enabled and scroll-load disabled]")
	...
	;
T100
	NEW CONF,STATE,OUT,ERR,ID,TREE,JSON
	DO RESET
	DO CONFDEF^MIOOS(.CONF)
	DO EQ^MIOTASSERT(+$GET(CONF("mioos","fs","textChunkBytes")),65536,"[MIOOST][T100][smaller text chunk default]")
	DO EQ^MIOTASSERT($GET(CONF("mioos","fs","maxTextEditBytes")),"","[MIOOST][T100][max text edit cap removed]")
	DO OK^MIOTASSERT($GET(CONF("mioos","route","fsTextSave"))="/api/mioos/fs/text-save","[MIOOST][T100][http text save route default]")
	DO INIT^MIOOSFS(.CONF)
	SET STATE("authenticated")=1,STATE("principal")="admin",STATE("roles")="admin"
	KILL OUT,ERR DO OK^MIOTASSERT($$WRITE^MIOOSFS(.STATE,$$HOMEID^MIOOSFS(),"t100-all-editable.txt","abc","text/plain",.OUT,.ERR),"[MIOOST][T100][write editable text setup]")
	SET ID=$GET(OUT("id"))
	...
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","stream.virtualized = false")&$$FILEHAS("public/mioos/app/mioos_explorer.js","maxEditBytes"),"[MIOOST][T100][full edit switches out of virtual view below cap]")
	...
	DO EQ^MIOTASSERT($$FILEHAS("routines/MIOOSWS.m","text_save_payload_too_large"),0,"[MIOOST][T100][backend edit byte cap removed]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","FSTEXTSAVE(DEV,CONF,REQ,CTX)"),"[MIOOST][T100][http save backend]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSST.m","OBJ(""routes"",""fsTextSave"")"),"[MIOOST][T100][boot exposes http save route]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/README.md","ROI 100 large text edit cap removal"),"[MIOOST][T100][readme docs]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/UI_Modules.md","ROI 100 text viewer integration note"),"[MIOOST][T100][ui docs]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/Backend_Table.md","ROI 100 text helper note"),"[MIOOST][T100][backend docs]")
	DO OK^MIOTASSERT($$FILEHAS("mioos_llm.md","ROI 100 handoff"),"[MIOOST][T100][llm docs]")
	QUIT
		;
T101
	DO RESET
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","previewOnOpen"),"[MIOOST][T101][markdown preview-on-open marker]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","stream.editing = !stream.previewOnOpen"),"[MIOOST][T101][markdown does not force edit mode]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_shell_ui.js","textEditNotice: function () { return ''; }"),"[MIOOST][T101][bottom feedback notice removed]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","stale_text_revision_conflict"),0,"[MIOOST][T101][no false stale conflict marker]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","text_chunk_loop_guard"),"[MIOOST][T101][large text loop guard]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","text_chunk_offset_stalled"),"[MIOOST][T101][large text stalled offset guard]")
	DO OK^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","text_file_too_large_for_browser_editor"),"[MIOOST][T101][browser edit cap message]")
	DO EQ^MIOTASSERT($$FILEHAS("public/mioos/app/mioos_explorer.js","stream: 'media'"),0,"[MIOOST][T101][media viewer no stream media url]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","mediaInitial206"),"[MIOOST][T101][synthetic media 206 opt-in only]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","ISWALL!(+ISMEDIA) DO SENDVFSRAW"),"[MIOOST][T101][media direct vfs streaming]")
	DO OK^MIOTASSERT($$FILEHAS("routines/MIOOSAPI.m","fs_text_save_requires_chunked")&$$FILEHAS("routines/MIOOSWS.m","fs_text_save_requires_chunked"),"[MIOOST][T101][oversized full-json save guard]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/README.md","ROI 102"),"[MIOOST][T101][readme docs]")
	DO OK^MIOTASSERT($$FILEHAS("docs/mioos/UI_Modules.md","ROI 102"),"[MIOOST][T101][ui docs]")
	DO OK^MIOTASSERT($$FILEHAS("mioos_llm.md","ROI 102"),"[MIOOST][T101][llm docs]")
	QUIT
