...
COMMANDJSON(CONF,REQ,CTX,STATE,PAYLOAD,OUTJSON,ERR)
	...
	IF CMD="fs.text.chunk" QUIT $$FSTEXTCHUNK(.STATE,.CONF,.TREE,.OUTJSON,.ERR)
	IF CMD="fs.text.save" QUIT $$FSTEXTSAVE(.STATE,.CONF,.TREE,.OUTJSON,.ERR)
	IF CMD="theme.delete" QUIT $$THEMEDEL(.STATE,.CONF,.TREE,.OUTJSON,.ERR)
...
FSTEXTCHUNK(STATE,CONF,TREE,OUTJSON,ERR)
	...
	SET ID=$SELECT($GET(TREE("id"))'="":$GET(TREE("id")),1:$GET(TREE("path")))
	IF $GET(TREE("offset"))'="",+$GET(TREE("offset"))<0 SET ERR("error")="invalid_offset",ERR("message")="Text chunk offset must be zero or greater" QUIT 0
	IF $GET(TREE("size"))'="",+$GET(TREE("size"))<1 SET ERR("error")="invalid_size",ERR("message")="Text chunk size must be positive" QUIT 0
...
FSTEXTSAVE(STATE,CONF,TREE,OUTJSON,ERR)
	...
	SET LIMIT=+$GET(CONF("mioos","fs","textChunkBytes"),65536) IF LIMIT<4096 SET LIMIT=4096
	IF LIMIT>65536 SET LIMIT=65536
	IF $ZLENGTH(DATA)>LIMIT SET ERR("error")="fs_text_save_requires_chunked",ERR("message")="Large text saves must use staged HTTP upload chunks" QUIT 0
	...
	QUIT 1
	;
THEMEDEL(STATE,CONF,TREE,OUTJSON,ERR)
	NEW OUT
	IF '$$DELETE^MIOOSTHEME(.STATE,.CONF,.TREE,.OUT,.ERR) QUIT 0
	SET OUTJSON=$$CMDOKJSON(.STATE,$GET(TREE("requestId")),"theme.delete","theme",.OUT)
	QUIT 1
	;
...
TABLEMUTATE(STATE,CONF,TREE,OUTJSON,ERR)
	...
	. SET OUT("dataset")=$GET(TREE("dataset")),OUT("action")=$GET(TREE("action")),OUT("message")=$GET(ERR("message"),$GET(ERR("error"))),OUT("mutationOnly")=1,OUT("refetch")=0
