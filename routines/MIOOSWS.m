...
FSWRITE(STATE,CONF,TREE,OUTJSON,ERR)
	NEW OUT,PARENT,NAME,DATA
	SET PARENT=$SELECT($GET(TREE("parent"))'="":$GET(TREE("parent")),1:"root")
	SET NAME=$GET(TREE("name"))
	SET DATA=$GET(TREE("content"))
	IF DATA="",$DATA(TREE("data")) SET DATA=$GET(TREE("data"))
	IF DATA="",$DATA(TREE("text")) SET DATA=$GET(TREE("text"))
	IF '$$WRITE^MIOOSFS(.STATE,PARENT,NAME,DATA,$GET(TREE("mime"),"text/plain"),.OUT,.ERR) QUIT 0
