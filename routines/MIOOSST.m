...
LOAD(CONF,REQ,CTX,STATE,ERR)
	...
	SET STATE("wsTransferPath")=$GET(CONF("mioos","route","wsTransfer"),"/ws/mioos/transfer")
	SET STATE("transferEnabled")=+$GET(CONF("mioos","transfer","enabled"),1)
	SET STATE("transferChunkSize")=+$GET(CONF("mioos","transfer","chunkSize"),16384)
	SET STATE("transferUploadWorkers")=+$GET(CONF("mioos","transfer","uploadWorkers"),1)
	SET STATE("transferDownloadWorkers")=+$GET(CONF("mioos","transfer","downloadWorkers"),1)
	SET STATE("transferResumeSeconds")=+$GET(CONF("mioos","transfer","resumeSeconds"),1800)
	SET STATE("transferActivePollMs")=+$GET(CONF("mioos","transfer","activePollMs"),25)
	SET STATE("transferIdlePollMs")=+$GET(CONF("mioos","transfer","idlePollMs"),180)
...
BOOTARY(STATE,CONF,OBJ)
	...
	SET OBJ("routes","transferWebsocket")=$GET(STATE("wsTransferPath"))
	...
	SET OBJ("transfer","enabled")=+$GET(STATE("transferEnabled"),1)
	SET OBJ("transfer","websocketPath")=$GET(STATE("wsTransferPath"))
	SET OBJ("transfer","chunkSize")=+$GET(STATE("transferChunkSize"),16384)
	SET OBJ("transfer","uploadWorkers")=+$GET(STATE("transferUploadWorkers"),1)
	SET OBJ("transfer","downloadWorkers")=+$GET(STATE("transferDownloadWorkers"),1)
	SET OBJ("transfer","resumeSeconds")=+$GET(STATE("transferResumeSeconds"),1800)
	SET OBJ("transfer","activePollMs")=+$GET(STATE("transferActivePollMs"),25)
	SET OBJ("transfer","idlePollMs")=+$GET(STATE("transferIdlePollMs"),180)
	SET OBJ("transfer","protocolVersion")=1
