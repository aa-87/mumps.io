...
IMPPARSE(ROOT,CSV,OUT,COMMIT,STATE)
	...
	;
CSVROW(HDR,LINE,ROW)
	NEW I,KEY,VAL,HARR,LARR
	KILL ROW,HARR,LARR
	DO CSVFLDS(HDR,.HARR)
	DO CSVFLDS(LINE,.LARR)
	SET I=0 FOR  SET I=$ORDER(HARR(I)) QUIT:I'>0  DO
	. SET KEY=$$TRIM^MIOUTIL($GET(HARR(I))),VAL=$$TRIM^MIOUTIL($GET(LARR(I)))
...
CSVFLDS(LINE,ARR)
	NEW I,C,VAL,N,INQ
	KILL ARR
	SET LINE=$GET(LINE),VAL="",N=1,INQ=0
	FOR I=1:1:$LENGTH(LINE) DO
	. SET C=$EXTRACT(LINE,I)
	. IF C="""",INQ,$EXTRACT(LINE,I+1)="""" SET VAL=VAL_"""",I=I+1 QUIT
	. IF C="""" SET INQ='INQ QUIT
	. IF C=",",'INQ SET ARR(N)=VAL,N=N+1,VAL="" QUIT
	. SET VAL=VAL_C
	SET ARR(N)=VAL
	QUIT
	;
