...
FSBLOB(DEV,CONF,REQ,CTX)
	...
	. IF METHOD'="head",ISWALL DO SENDVFSRAW(.DEV,.CONF,RID,RS,RLEN,.SERR)
	. IF METHOD'="head",'ISWALL DO SENDVFS(.DEV,.CONF,RID,RS,RLEN,+ISMEDIA,MEDIAWARM,.SERR)
	...
	IF METHOD'="head",ISWALL DO SENDVFSRAW(.DEV,.CONF,RID,0,SIZE,.SERR)
	IF METHOD'="head",'ISWALL DO SENDVFS(.DEV,.CONF,RID,0,SIZE,+ISMEDIA,MEDIAWARM,.SERR)
	IF $DATA(SERR("error")) DO LOGSTREAM(.CTX,RID,0,SIZE,.SERR)
	SET CTX("status")=200
	QUIT
	;
THEMEWALL(RID)
	IF +$$METAFIELD^MIOOSFS($GET(RID),"themeWallpaper") QUIT 1
	IF $$METAFIELD^MIOOSFS($GET(RID),"sourceAsset")'="" QUIT 1
	QUIT 0
	;
SENDVFSRAW(DEV,CONF,RID,OFFSET,LEN,ERR)
	NEW TOTAL,CHSZ,POS,END,IDX,SEG,SEGBYTES,OFFINSEG,TAKE,PART,SEND
	...
	. IF SEGBYTES<1 SET ERR("error")="fs_wallpaper_read_failed",ERR("detail")=IDX QUIT
	. SET OFFINSEG=(POS#CHSZ)+1
	. SET TAKE=(SEGBYTES-OFFINSEG)+1
	. IF TAKE>(END-POS+1) SET TAKE=(END-POS+1)
	. IF TAKE<1 SET ERR("error")="fs_wallpaper_read_stalled",ERR("detail")=POS QUIT
	. SET PART=$SELECT(OFFINSEG=1&(TAKE=SEGBYTES):SEG,1:$ZEXTRACT(SEG,OFFINSEG,OFFINSEG+TAKE-1))
	. SET SEND=$ZLENGTH(PART)
	. IF SEND<1 SET ERR("error")="fs_wallpaper_write_stalled",ERR("detail")=POS QUIT
...
ROOTDATASIZE(ROOT)
	...
	QUIT SIZE
	;
SENDROOTRAW(DEV,ROOT)
	NEW IDX
	SET IDX=0 FOR  SET IDX=$ORDER(@ROOT@("DATA",IDX)) QUIT:IDX'>0  DO WOUT^MIOSTATIC(.DEV,$GET(@ROOT@("DATA",IDX)))
	QUIT
	;
...
THEMEPUBLICASSET(DEV,CONF,REQ,CTX)
	NEW ID,USER,ROOT,META,MIME,FN,SIZE,HEAD,METHOD,IDX,CH,ACTUAL
	...
	IF METHOD'="head" DO SENDROOTRAW(.DEV,ROOT)
	SET CTX("status")=200
	QUIT
	;
THEMEASSET(DEV,CONF,REQ,CTX)
	NEW STATE,ERR,USER,ID,ROOT,META,MIME,FN,SIZE,HEAD,METHOD,IDX,PUBLIC,CH,ACTUAL
	IF '$$LOAD^MIOOSST(.CONF,.REQ,.CTX,.STATE,.ERR) DO  QUIT
	. DO RESPERR(.DEV,.CONF,500,"theme_asset_state_error",$GET(ERR("error")),.CTX)
	SET ID=$GET(REQ("query","id"))
	IF ID="" DO  QUIT
