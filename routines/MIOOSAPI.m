...
SENDVFS(DEV,CONF,RID,OFFSET,LEN,ISMEDIA,MEDIAWARM,ERR)
	NEW POS,END,IDX,SEG,SEGBYTES,SEGSTART,SEGEND,FROM,TO,PART,SEND,TOTAL,SENT,SENDBUF,SENDCH,OUTCH,FLUSHLEN,FIRSTCH
	SET ERR("routine")="MIOOSAPI"
	SET TOTAL=$$DATASIZE^MIOOSFS(RID)
	...
	SET LEN=+$GET(LEN)
	IF (LEN<1)!(TOTAL<1) QUIT
	IF OFFSET'<TOTAL QUIT
	SET END=OFFSET+LEN-1
	IF END>(TOTAL-1) SET END=TOTAL-1
	SET LEN=(END-OFFSET)+1,SENDBUF="",SEGSTART=0,SENT=0,IDX=0
	FOR  SET IDX=$ORDER(^MIO("MIOOS","FS","DATA",RID,IDX)) QUIT:(IDX'>0)!(SEGSTART>END)!$DATA(ERR("error"))  DO
	...
	. SET SEGEND=SEGSTART+SEGBYTES-1
	. IF SEGEND<OFFSET SET SEGSTART=SEGSTART+SEGBYTES QUIT
	. SET FROM=$SELECT(OFFSET>SEGSTART:(OFFSET-SEGSTART)+1,1:1)
	. SET TO=$SELECT(END<SEGEND:(END-SEGSTART)+1,1:SEGBYTES)
	. IF TO<FROM SET SEGSTART=SEGSTART+SEGBYTES QUIT
	. SET PART=$ZEXTRACT(SEG,FROM,TO)
	. SET SEND=$ZLENGTH(PART)
	. IF SEND<1 SET ERR("error")="fs_blob_read_stalled",ERR("detail")=SEGSTART QUIT
	. SET SENT=SENT+SEND,SENDBUF=SENDBUF_PART
	...
	. . IF FLUSHLEN<1 SET ERR("error")="fs_blob_write_stalled",ERR("detail")=SEGSTART QUIT
	. . DO WOUT^MIOSTATIC(.DEV,OUTCH)
	. . SET SENDBUF=$ZEXTRACT(SENDBUF,FLUSHLEN+1,$ZLENGTH(SENDBUF))
	. . SET FIRSTCH=0
	. SET SEGSTART=SEGSTART+SEGBYTES
	IF '$DATA(ERR("error")),SENT<LEN SET ERR("error")="fs_blob_read_short",ERR("detail")=SENT_"/"_LEN
