...
FSBLOB(DEV,CONF,REQ,CTX)
	NEW STATE,ERR,ID,RID,HEAD,SIZE,MIME,NAME,DL,RS,RE,RLEN,RNG,OK,ETAG,METHOD,H304,H416,H206,H200,SERR,STREAM,MEDIAINIT,MEDIAWARM,WARMFLAG
	IF '$$LOAD^MIOOSST(.CONF,.REQ,.CTX,.STATE,.ERR) DO  QUIT
	...
	SET MEDIAWARM=+$GET(CONF("mioos","download","mediaWarmupBytes"),1048576)
	IF MEDIAWARM<16384 SET MEDIAWARM=16384
	SET WARMFLAG=$SELECT(STREAM="media":1,1:0)
	...
	. IF METHOD'="head" DO SENDVFS(.DEV,.CONF,RID,RS,RLEN,.SERR,WARMFLAG)
	...
	. SET H206("Content-Length")=RLEN
	. DO RESPHEAD^MIOSTATIC(.DEV,.CONF,206,.H206,$GET(CTX("request_id")))
	. DO SENDVFS(.DEV,.CONF,RID,RS,RLEN,.SERR,1)
	. IF $DATA(SERR("error")) DO LOGSTREAM(.CTX,RID,RS,RLEN,.SERR)
	. SET CTX("status")=206
	MERGE H200=HEAD
	SET H200("Content-Length")=SIZE
	DO RESPHEAD^MIOSTATIC(.DEV,.CONF,200,.H200,$GET(CTX("request_id")))
	IF METHOD'="head" DO SENDVFS(.DEV,.CONF,RID,0,SIZE,.SERR,WARMFLAG)
...
LOGSTREAM(CTX,RID,OFFSET,LEN,ERR)
	...
	QUIT
	;
SENDVFS(DEV,CONF,RID,OFFSET,LEN,ERR,WARMUP)
	NEW CHSZ,POS,END,IDX,SEG,SEGBYTES,OFFINSEG,TAKE,PART,SEND,TOTAL,SENDBUF,SENDCH,OUTCH,FLUSHLEN,WARMCH,WARMED
	...
	IF SENDCH>1048576 SET SENDCH=1048576
	SET WARMCH=+$GET(CONF("mioos","download","mediaWarmupBytes"),1048576)
	IF WARMCH<16384 SET WARMCH=16384
	IF WARMCH>SENDCH SET WARMCH=SENDCH
	SET WARMED=0
	SET POS=+$GET(OFFSET)
	IF POS<0 SET POS=0
	SET LEN=+$GET(LEN)
	...
	. IF +$GET(WARMUP),'WARMED,$ZLENGTH(SENDBUF)'<WARMCH DO
	. . SET OUTCH=$ZEXTRACT(SENDBUF,1,WARMCH)
	...
	. . SET SENDBUF=$ZEXTRACT(SENDBUF,WARMCH+1,$ZLENGTH(SENDBUF))
	. . SET WARMED=1
	. FOR  QUIT:$ZLENGTH(SENDBUF)<SENDCH  DO
	. . SET OUTCH=$ZEXTRACT(SENDBUF,1,SENDCH)
	. . SET FLUSHLEN=$ZLENGTH(OUTCH)
	. . IF FLUSHLEN<1 SET ERR("error")="fs_blob_write_stalled",ERR("detail")=POS QUIT
	. . DO WOUT^MIOSTATIC(.DEV,OUTCH)
	. . SET SENDBUF=$ZEXTRACT(SENDBUF,SENDCH+1,$ZLENGTH(SENDBUF))
	. SET POS=POS+SEND
	IF '$DATA(ERR("error")),$ZLENGTH(SENDBUF)>0 DO WOUT^MIOSTATIC(.DEV,SENDBUF)
	QUIT
	;
	;
ISTRUE(X)
