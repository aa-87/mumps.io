...
CANON(CODE)
	...
	SET CODE=$EXTRACT(CODE,1,2)
	IF CODE="sp" SET CODE="es"
	QUIT CODE
	;
LABEL(CODE)
	SET CODE=$$CANON($GET(CODE))
...
CATALOG(CODE,ROOT)
	...
	SET @ROOT@("dialog.cancel")=$$TXT(CODE,"dialog.cancel","Cancel")
		SET @ROOT@("workspace.label")=$$TXT(CODE,"workspace.label","Workspaces")
		SET @ROOT@("workspace.current")=$$TXT(CODE,"workspace.current","Current workspace")
		SET @ROOT@("workspace.moveTo")=$$TXT(CODE,"workspace.moveTo","Move to workspace")
		SET @ROOT@("workspace.main")=$$TXT(CODE,"workspace.main","Desktop")
		SET @ROOT@("workspace.main.description")=$$TXT(CODE,"workspace.main.description","Primary shell workspace for everyday apps and the desktop surface")
		SET @ROOT@("workspace.files")=$$TXT(CODE,"workspace.files","Files")
		SET @ROOT@("workspace.files.description")=$$TXT(CODE,"workspace.files.description","Explorer, transfers, and file-focused windows")
		SET @ROOT@("workspace.operations")=$$TXT(CODE,"workspace.operations","Operations")
		SET @ROOT@("workspace.operations.description")=$$TXT(CODE,"workspace.operations.description","Security, diagnostics, and operational control surfaces")
		SET @ROOT@("workspace.studio")=$$TXT(CODE,"workspace.studio","Studio")
		SET @ROOT@("workspace.studio.description")=$$TXT(CODE,"workspace.studio.description","Theme work, modules, and catalog windows")
...
;;workspace.current=مساحة العمل الحالية
;;workspace.moveTo=نقل إلى مساحة العمل
;;workspace.main=سطح المكتب
;;workspace.main.description=مساحة العمل الأساسية للتطبيقات اليومية وسطح المكتب
;;workspace.files=الملفات
;;workspace.files.description=المستكشف وعمليات النقل والنوافذ المتعلقة بالملفات
;;workspace.operations=العمليات
;;workspace.operations.description=مركز الأمان والتشخيص وواجهات التحكم التشغيلية
;;workspace.studio=الاستوديو
;;workspace.studio.description=السمة والوحدات ونوافذ الكتالوج
...
;;workspace.moveTo=Mover al espacio de trabajo
;;workspace.main=Escritorio
;;workspace.main.description=Espacio principal para aplicaciones diarias y la superficie del escritorio
;;workspace.files=Archivos
;;workspace.files.description=Explorador, transferencias y ventanas centradas en archivos
;;workspace.operations=Operaciones
;;workspace.operations.description=Seguridad, diagnósticos y superficies de control operativo
;;workspace.studio=Studio
;;workspace.studio.description=Temas, módulos y ventanas del catálogo
