...
CATALOG(CODE,ROOT)
	...
	SET @ROOT@("app.app-catalog.title")=$$TXT(CODE,"app.app-catalog.title","App Catalog")
	SET @ROOT@("app.app-catalog.subtitle")=$$TXT(CODE,"app.app-catalog.subtitle","Installed modules, launch policy, and built-in surfaces")
	SET @ROOT@("module.notes.title")=$$TXT(CODE,"module.notes.title","Notes")
	SET @ROOT@("module.notes.subtitle")=$$TXT(CODE,"module.notes.subtitle","A lightweight notes surface for quick capture inside MIOOS")
	SET @ROOT@("module.notes.description")=$$TXT(CODE,"module.notes.description","Capture a short scratch note, track a few pinned cards, and keep module state inside the shell contract.")
	SET @ROOT@("module.ops.title")=$$TXT(CODE,"module.ops.title","Ops Center")
	SET @ROOT@("module.ops.subtitle")=$$TXT(CODE,"module.ops.subtitle","A built-in operational summary surface for the current shell session")
	SET @ROOT@("module.ops.description")=$$TXT(CODE,"module.ops.description","Review session identity, transport posture, and desktop contract metadata from one reusable module host.")
	QUIT
	;
