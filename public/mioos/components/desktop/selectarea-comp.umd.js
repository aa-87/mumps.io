(function () {
  window["selectarea-comp"] = {
    data() {
      return {};
    },
    mounted() {
      var isMouseDown = false;
      var selectionArea = document.querySelector('[data-attr="selectionArea"]')
      var startX, startY, endX, endY;

      document.addEventListener("mousedown", function (event) {
        if (
          event.target.localName == "html" ||
          event.target.id == "folders" ||
          event.target.id == "error" ||
          event.target.id.includes("my")
        ) {
          isMouseDown = true;
      ...
      document.addEventListener("mousemove", function (event) {
        if (
          (isMouseDown && event.target.localName == "html") ||
          event.target.id == "folders" ||
          event.target.id == "startmenu" ||
          event.target.id == "error" ||
          event.target.id.includes("my")
        ) {
          endX = event.clientX;
    ...
    methods: {},
    template: `<div data-attr="selectionArea" class="absolute z-[300] bg-[#0453ff]/50 border border-[#0453ff] pointer-events-none"></div>`,
  };
})();
