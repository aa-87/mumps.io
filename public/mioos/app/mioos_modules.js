(function () {
  ...
  function bootRegistry(vm) { return (((vm || {}).boot || {}).uiModules) || { modules: [], components: [], examples: [], tableDefinitions: [], contract: 'mioos-ui-module-v2' }; }
  ...
  function tableDefinitionRows(vm) { return uniqueRows(toList(bootRegistry(vm).tableDefinitions), function (row) { return row && (row.key || row.dataset || row.title); }); }
  function defaultTableDefinition() { return { key: 'example_table', title: 'Example Table', dataset: 'example_table', category: 'Operations', icon: '▤', description: 'Server-authored table module created from MIOOS.', schema: { columns: [{ key: 'name', label: 'Name', type: 'text', width: 220, resizable: 1 }, { key: 'status', label: 'Status', type: 'select', width: 140, resizable: 1 }, { key: 'updated', label: 'Updated', type: 'date', width: 150, resizable: 1 }, { key: 'notes', label: 'Notes', type: 'textarea', width: 260, resizable: 1 }] }, validation: { fields: { name: { required: 1, maxLength: 120 }, status: { required: 1, enum: ['Active', 'Pending', 'Review'] }, updated: { date: 1 }, notes: { maxLength: 2048 } } }, rows: [{ id: 'example-1', name: 'Demo row', status: 'Active', updated: '2026-05-02', notes: 'Created through the server-side table module library.' }] }; }
  function ensureDraftShape(def) {
    def = def || defaultTableDefinition();
    def.schema = def.schema || {};
    if (!Array.isArray(def.schema.columns)) def.schema.columns = [];
  ...
  function makeDraftRow(def) {
    def = ensureDraftShape(def);
    var row = { id: (def.dataset || def.key || 'row') + '-' + (def.rows.length + 1) };
    def.schema.columns.forEach(function (col) { if (col && col.key && typeof row[col.key] === 'undefined') row[col.key] = ''; });
    return row;
  }
  function previewColumns(preview) { return toList((((preview || {}).query || {}).schema || {}).columns); }
  function previewRows(preview) { return toList(((preview || {}).query || {}).rows); }
  ...
  function downloadJsonFile(name, value) {
    ...
    try {
      ...
      a.href = url;
      a.download = String(name || 'table-module.json').replace(/[^A-Za-z0-9._-]+/g, '_');
  ...
  function validateTableDefinition(def) {
    var errors = {}, seen = {};
    def = def || {};
    if (!String(def.key || '').trim()) errors.key = 'Module key is required.';
    if (!String(def.title || '').trim()) errors.title = 'Title is required.';
    if (!def.schema || !Array.isArray(def.schema.columns) || !def.schema.columns.length) errors['schema.columns'] = 'At least one column is required.';
    (def.schema && Array.isArray(def.schema.columns) ? def.schema.columns : []).forEach(function (column, index) {
      var key = String((column || {}).key || '').trim();
      var field = 'schema.columns.' + (index + 1) + '.key';
      if (!key) errors[field] = 'Column key is required.';
      else if (!/^[A-Za-z][A-Za-z0-9_]*$/.test(key)) errors[field] = 'Use letters, numbers, and underscores; start with a letter.';
      else if (seen[key]) errors[field] = 'Column key must be unique.';
      seen[key] = 1;
    });
    return errors;
  }
  function hasErrors(errors) { return !!Object.keys(errors || {}).length; }
  ...
  var methods = {
    ...
    uiModuleTableDefinitionRows: function () { return tableDefinitionRows(this); },
    ...
    uiModuleTableRoute: function () { return ((((this.boot || {}).routes || {}).moduleTable) || '/api/mioos/modules/table'); },
    uiModuleTableRequest: function (action, payload) { var body = Object.assign({}, payload || {}, { action: action || 'list' }); return fetch(this.uiModuleTableRoute(), { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(function (response) { return response.text().then(function (text) { var result = null; if (text) { try { result = JSON.parse(text); } catch (ignore) { result = null; } } if (!response.ok) throw new Error('Table module action failed: HTTP ' + response.status + (text ? ' — ' + text.slice(0, 160) : '')); if (!text) throw new Error('Table module action returned an empty response. Check MODULETABLE^MIOOSAPI route registration.'); if (!result) throw new Error('Table module action returned invalid JSON.'); return result; }); }).then(function (result) { if (!result || result.ok === false) { var err = new Error((result && (result.message || result.error)) || 'Table module action failed'); err.payload = result || {}; throw err; } return result; }); },
  ...
  function registerCatalog(app) {
    app.component('mioos-surface-ui-modules', {
      props: ['window'],
      data: function () { return { query: '', category: 'all', source: 'all', mode: 'modules', layout: 'cards', error: '', loading: false, tableLoading: false, tableStatus: '', tableError: '', tableEditorOpen: false, tableDraft: defaultTableDefinition(), tableExportText: '', tablePreview: null, tableRevisions: [], fieldErrors: {} }; },
      computed: {
        ...
        tableDefinitions: function () { return this.vm.uiModuleTableDefinitionRows ? this.vm.uiModuleTableDefinitionRows() : []; },
        rows: function () { return this.mode === 'components' ? this.components : (this.mode === 'examples' ? this.examples : (this.mode === 'tableDefinitions' ? this.tableDefinitions : this.modules)); },
        ...
        libraryCount: function () { return ((this.vm.uiModuleRegistry() || {}).tableModuleLibrary || {}).count || this.tableDefinitions.length; },
        draftColumns: function () { return (((this.tableDraft || {}).schema || {}).columns) || []; },
        draftRows: function () { return ((this.tableDraft || {}).rows) || []; },
        previewColumns: function () { return previewColumns(this.tablePreview); },
        previewRows: function () { return previewRows(this.tablePreview); }
      },
      mounted: function () { this._mioosTableEditorKeydown = this.onCatalogKeydown.bind(this); window.addEventListener('keydown', this._mioosTableEditorKeydown, true); if (!this.modules.length && this.vm.uiModuleRefreshCatalog) this.refresh(); },
      beforeUnmount: function () { if (this._mioosTableEditorKeydown) window.removeEventListener('keydown', this._mioosTableEditorKeydown, true); },
      methods: {
        ...
        resetFilters: function () { this.query = ''; this.category = 'all'; this.source = 'all'; },
        setMode: function (mode) { this.mode = mode; this.category = 'all'; this.source = 'all'; },
        ...
        tableDefinitionAction: function (action, payload) { var self = this; self.tableLoading = true; self.tableError = ''; self.fieldErrors = {}; return self.vm.uiModuleTableRequest(action, payload || {}).then(function (result) { self.tableStatus = result.message || (action + ' completed'); return result; }).catch(function (err) { var payload = err && err.payload; self.fieldErrors = (payload && payload.fieldErrors) || {}; self.tableError = (payload && (payload.message || payload.error)) || (err && err.message) || 'Table module action failed'; return { ok: false, error: self.tableError, fieldErrors: self.fieldErrors }; }).finally(function () { self.tableLoading = false; }); },
        newTableDefinition: function () { this.tableDraft = ensureDraftShape(defaultTableDefinition()); this.tablePreview = null; this.tableRevisions = []; this.tableExportText = ''; this.fieldErrors = {}; this.tableError = ''; this.tableEditorOpen = true; },
        editTableDefinition: function (row) { var self = this; return this.tableDefinitionAction('export', { key: row.key || row.id }).then(function (result) { if (!result || result.ok === false) return result; self.tableDraft = ensureDraftShape(clone(result.definition || defaultTableDefinition())); self.tablePreview = null; self.tableRevisions = []; self.tableExportText = ''; self.tableEditorOpen = true; return result; }); },
        saveTableDefinition: function () { var self = this; ensureDraftShape(this.tableDraft); self.fieldErrors = validateTableDefinition(this.tableDraft); if (hasErrors(self.fieldErrors)) { self.tableError = 'Resolve table definition validation errors before saving.'; return Promise.resolve({ ok: false, fieldErrors: self.fieldErrors }); } return this.tableDefinitionAction('save', { definition: this.tableDraft }).then(function (result) { if (!result || result.ok === false) return result; self.tableEditorOpen = false; return self.refresh(); }); },
        previewTableDefinition: function () { var self = this; ensureDraftShape(this.tableDraft); self.fieldErrors = validateTableDefinition(this.tableDraft); if (hasErrors(self.fieldErrors)) { self.tableError = 'Resolve table definition validation errors before preview.'; return Promise.resolve({ ok: false, fieldErrors: self.fieldErrors }); } return this.tableDefinitionAction('preview', { definition: this.tableDraft }).then(function (result) { if (!result || result.ok === false) return result; self.tablePreview = result; return result; }); },
        exportTableDefinition: function (row) { var self = this; if (!row) { var draft = ensureDraftShape(this.tableDraft); this.tableExportText = JSON.stringify(draft, null, 2); downloadJsonFile((draft.key || draft.dataset || 'table-module') + '.json', this.tableExportText); this.tableStatus = 'Draft JSON exported locally'; return Promise.resolve({ ok: true }); } var key = row.key || row.id; return this.tableDefinitionAction('export', { key: key }).then(function (result) { if (!result || result.ok === false) return result; self.tableExportText = JSON.stringify(result.definition || {}, null, 2); downloadJsonFile((key || 'table-module') + '.json', self.tableExportText); return result; }); },
        importTableDefinition: function () { var self = this, parsed; self.fieldErrors = {}; self.tableError = ''; try { parsed = JSON.parse(this.tableExportText || '{}'); } catch (err) { this.tableError = 'Import JSON is invalid.'; return Promise.resolve({ ok: false, error: this.tableError }); } parsed = ensureDraftShape(parsed); self.fieldErrors = validateTableDefinition(parsed); if (hasErrors(self.fieldErrors)) { self.tableDraft = parsed; self.tableError = 'Import JSON failed validation. Fix the highlighted fields before importing.'; return Promise.resolve({ ok: false, fieldErrors: self.fieldErrors }); } return this.tableDefinitionAction('import', { definition: parsed }).then(function (result) { if (!result || result.ok === false) return result; self.tableEditorOpen = false; return self.refresh(); }); },
        loadTableRevisions: function (row) { var self = this; return this.tableDefinitionAction('revisions', { key: (row && (row.key || row.id)) || this.tableDraft.key }).then(function (result) { if (!result || result.ok === false) return result; self.tableRevisions = toList(result.revisions); if (!self.tableRevisions.length) self.tableStatus = 'No revisions found for this table module'; return result; }); },
        rollbackTableDefinition: function (revision) { var self = this; if (!revision || !revision.id) return Promise.resolve(); return this.tableDefinitionAction('rollback', { key: this.tableDraft.key, revisionId: revision.id }).then(function (result) { if (!result || result.ok === false) return result; self.tableRevisions = []; return self.refresh(); }); },
        addTableColumn: function () { ensureDraftShape(this.tableDraft); this.tableDraft.schema.columns.push({ key: 'field' + (this.tableDraft.schema.columns.length + 1), label: 'Field ' + (this.tableDraft.schema.columns.length + 1), type: 'text', width: 140, resizable: 1 }); },
        removeTableColumn: function (index) { ensureDraftShape(this.tableDraft); if (this.tableDraft.schema.columns.length <= 1) { this.tableError = 'At least one column is required.'; return; } this.tableDraft.schema.columns.splice(index, 1); },
        addTableRow: function () { ensureDraftShape(this.tableDraft); this.tableDraft.rows.push(makeDraftRow(this.tableDraft)); },
        removeTableRow: function (index) { ensureDraftShape(this.tableDraft); this.tableDraft.rows.splice(index, 1); if (!this.tableDraft.rows.length) this.addTableRow(); },
        typeOptions: function () { return ['text', 'textarea', 'select', 'multiselect', 'boolean', 'date', 'datetime', 'time', 'number', 'integer', 'currency', 'percent', 'email', 'phone', 'url', 'state', 'gender', 'mrn', 'zip']; },
        inputTypeForColumn: function (column) { var type = String((column || {}).type || 'text').toLowerCase(); if (type === 'number' || type === 'integer' || type === 'currency' || type === 'percent') return 'number'; if (type === 'date' || type === 'dob') return 'date'; if (type === 'datetime' || type === 'timestamp') return 'datetime-local'; if (type === 'time') return 'time'; if (type === 'email') return 'email'; if (type === 'phone' || type === 'tel') return 'tel'; if (type === 'url') return 'url'; return type === 'boolean' ? 'checkbox' : 'text'; },
        tableFieldError: function (field) { return (this.fieldErrors || {})[field] || ''; },
        onCatalogKeydown: function (event) { if (!event || event.key !== 'Escape') return; if (this.tableEditorOpen) { event.preventDefault(); event.stopPropagation(); this.closeTableEditor(); } },
        closeTableEditor: function () { this.tableEditorOpen = false; this.tableLoading = false; }
      },
      template: '' +
        '<div class="mioos-surface mioos-ui-module-shell mioos-ui-module-catalog-v3">' +
          '<header class="mioos-ui-module-head"><div><strong>App Catalogue + UI Modules</strong><span>[[ contract ]] • server-authored launcher and table module library</span></div><div class="mioos-ui-module-actions"><button type="button" class="mioos-btn" :class="{ active: layout === \'list\' }" @click="layout = layout === \'cards\' ? \'list\' : \'cards\'">[[ layout === \'cards\' ? \'List\' : \'Cards\' ]]</button><button type="button" class="mioos-btn is-primary" @click="newTableDefinition">New table module</button><button type="button" class="mioos-btn" :disabled="loading" @click="refresh">Refresh</button></div></header>' +
          '<nav class="mioos-ui-module-tabs" aria-label="Module catalog sections"><button type="button" :class="{ active: mode === \'modules\' }" @click="setMode(\'modules\')">Modules <span>[[ modules.length ]]</span></button><button type="button" :class="{ active: mode === \'tableDefinitions\' }" @click="setMode(\'tableDefinitions\')">Table Modules <span>[[ libraryCount ]]</span></button><button type="button" :class="{ active: mode === \'components\' }" @click="setMode(\'components\')">Components <span>[[ components.length ]]</span></button><button type="button" :class="{ active: mode === \'examples\' }" @click="setMode(\'examples\')">Examples <span>[[ examples.length ]]</span></button></nav>' +
          '<section class="mioos-ui-module-filterbar"><label><span>Search</span><input v-model="query" type="search" placeholder="Find modules, examples, components"></label><label><span>Category</span><select v-model="category"><option value="all">All categories</option><option v-for="item in categories" :key="item" :value="item">[[ item ]]</option></select></label><label><span>Source</span><select v-model="source"><option value="all">All sources</option><option v-for="item in sources" :key="item" :value="item">[[ item ]]</option></select></label><button type="button" class="mioos-btn" @click="resetFilters">Clear</button></section>' +
          '<p v-if="error" class="mioos-ui-module-error" role="alert">[[ error ]]</p><p v-if="tableError" class="mioos-ui-module-error" role="alert">[[ tableError ]]</p><p v-else-if="tableStatus" class="mioos-ui-module-status" role="status">[[ tableStatus ]]</p><p v-if="loading" class="mioos-ui-module-empty">Loading catalogue…</p><p v-else-if="!visibleRows.length" class="mioos-ui-module-empty">No [[ mode ]] match the current filters.</p>' +
          '<section v-else class="mioos-ui-module-grid" :class="{ \'is-list\': layout === \'list\' }"><article v-for="row in visibleRows" :key="keyOf(row)" class="mioos-ui-module-card" tabindex="0" role="button" @keyup.enter="launch(row)" @keyup.space.prevent="launch(row)" @click="launch(row)"><div class="mioos-ui-module-icon">[[ row.icon || \'▣\' ]]</div><div><strong>[[ row.title || row.name || keyOf(row) ]]</strong><span>[[ row.description || row.path || row.dataset || \'MIOOS module surface\' ]]</span><footer><em>[[ categoryOf(row) ]]</em><b>[[ badge(row) ]]</b></footer><nav v-if="mode === \'tableDefinitions\'" class="mioos-ui-module-card-actions"><button type="button" class="mioos-btn" @click.stop="editTableDefinition(row)">Edit</button><button type="button" class="mioos-btn" @click.stop="exportTableDefinition(row)">Export</button><button type="button" class="mioos-btn" @click.stop="editTableDefinition(row); loadTableRevisions(row)">Revisions</button></nav></div></article></section>' +
          '<section v-if="tableEditorOpen" class="mioos-ui-dialog-backdrop" role="presentation"><div class="mioos-ui-dialog mioos-table-module-editor" role="dialog" aria-modal="true" aria-labelledby="mtm-title">' +
            '<header><div><strong id="mtm-title">Table module definition</strong><span>Build a server-authored table module without writing frontend code.</span></div><button type="button" @click="closeTableEditor" aria-label="Close">×</button></header>' +
            '<main>' +
              '<div class="mioos-form-grid"><label :class="{ invalid: tableFieldError(&quot;key&quot;) }"><span>Module key</span><input v-model.trim="tableDraft.key" type="text" placeholder="example_table"><b v-if="tableFieldError(&quot;key&quot;)">[[ tableFieldError("key") ]]</b></label><label :class="{ invalid: tableFieldError(&quot;title&quot;) }"><span>Title</span><input v-model.trim="tableDraft.title" type="text"><b v-if="tableFieldError(&quot;title&quot;)">[[ tableFieldError("title") ]]</b></label><label><span>Dataset</span><input v-model.trim="tableDraft.dataset" type="text"></label><label><span>Category</span><input v-model.trim="tableDraft.category" type="text"></label><label><span>Icon</span><input v-model.trim="tableDraft.icon" type="text"></label><label class="span-full"><span>Description</span><textarea v-model="tableDraft.description" rows="2"></textarea></label></div>' +
              '<section class="mioos-table-module-columns"><div class="mioos-table-module-section-head"><div><h3>Columns</h3><small>These become schema.columns in the MUMPS table contract.</small></div><button type="button" class="mioos-btn" @click="addTableColumn">Add column</button></div><p v-if="tableFieldError(&quot;schema.columns&quot;)" class="mioos-ui-module-error">[[ tableFieldError("schema.columns") ]]</p><div v-for="(column, index) in draftColumns" :key="index" class="mioos-table-module-column-row"><label :class="{ invalid: tableFieldError(&quot;schema.columns.&quot; + (index + 1) + &quot;.key&quot;) }"><span>Key</span><input v-model.trim="column.key" placeholder="key"><b v-if="tableFieldError(&quot;schema.columns.&quot; + (index + 1) + &quot;.key&quot;)">[[ tableFieldError("schema.columns." + (index + 1) + ".key") ]]</b></label><label><span>Label</span><input v-model.trim="column.label" placeholder="Label"></label><label><span>Type</span><select v-model="column.type"><option v-for="type in typeOptions()" :key="type" :value="type">[[ type ]]</option></select></label><label><span>Width</span><input v-model.number="column.width" type="number" min="48"></label><button type="button" class="mioos-btn danger" @click="removeTableColumn(index)">Remove</button></div></section>' +
              '<section class="mioos-table-module-rows"><div class="mioos-table-module-section-head"><div><h3>Sample rows</h3><small>Preview and saved module seed data; production tables can later use MUMPS globals directly.</small></div><button type="button" class="mioos-btn" @click="addTableRow">Add row</button></div><div v-for="(row, rowIndex) in draftRows" :key="rowIndex" class="mioos-table-module-row-editor"><label><span>ID</span><input v-model.trim="row.id" type="text"></label><label v-for="column in draftColumns" :key="column.key"><span>[[ column.label || column.key ]]</span><textarea v-if="column.type === &quot;textarea&quot;" v-model="row[column.key]" rows="2"></textarea><select v-else-if="column.type === &quot;boolean&quot;" v-model="row[column.key]"><option value="">Blank</option><option value="true">true</option><option value="false">false</option></select><input v-else v-model="row[column.key]" :type="inputTypeForColumn(column)"></label><div class="mioos-table-module-row-actions"><button type="button" class="mioos-btn danger" @click="removeTableRow(rowIndex)">Remove row</button></div></div></section>' +
              '<section class="mioos-table-module-json"><div class="mioos-table-module-section-head"><div><h3>Import / export JSON</h3><small>Use this for copyable table-module definitions and source control review.</small></div></div><textarea v-model="tableExportText" rows="6" placeholder="Exported table module JSON appears here. Paste JSON here to import."></textarea></section>' +
              '<section v-if="tablePreview" class="mioos-table-module-preview"><h3>Preview</h3><dl><dt>Module</dt><dd>[[ tablePreview.module.title ]]</dd><dt>Dataset</dt><dd>[[ tablePreview.query.dataset ]]</dd><dt>Rows</dt><dd>[[ tablePreview.query.recordsTotal ]] sample row(s)</dd></dl><table v-if="previewRows.length"><thead><tr><th v-for="column in previewColumns" :key="column.key">[[ column.label || column.key ]]</th></tr></thead><tbody><tr v-for="row in previewRows" :key="row.id"><td v-for="column in previewColumns" :key="column.key">[[ row[column.key] ]]</td></tr></tbody></table></section>' +
              '<section v-if="tableRevisions.length" class="mioos-table-module-revisions"><h3>Revisions</h3><ul><li v-for="rev in tableRevisions" :key="rev.id"><span><strong>[[ rev.id ]]</strong><small>[[ rev.reason || "revision" ]] • [[ rev.title || tableDraft.title ]]</small></span><button type="button" class="mioos-btn" @click="rollbackTableDefinition(rev)">Rollback</button></li></ul></section>' +
            '</main><footer><button type="button" class="mioos-btn" @click="previewTableDefinition" :disabled="tableLoading">Preview</button><button type="button" class="mioos-btn" @click="exportTableDefinition" :disabled="tableLoading">Export JSON</button><button type="button" class="mioos-btn" @click="importTableDefinition" :disabled="tableLoading">Import JSON</button><button type="button" class="mioos-btn" @click="loadTableRevisions" :disabled="tableLoading">Revisions</button><button type="button" class="mioos-btn is-primary" @click="saveTableDefinition" :disabled="tableLoading">[[ tableLoading ? "Saving…" : "Save and register" ]]</button></footer></div></section>' +
        '</div>'
    });
  }
