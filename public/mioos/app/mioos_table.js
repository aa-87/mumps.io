  ...
  var methods = {
    ...
    backendTableDefaultState: function (tableId) {
      return {
        ...
        cellContextMenu: { open: false, left: 0, top: 0 },
    ...
    backendTableDismissToast: function (tableId) {
      var state = this.backendTableState(tableId);
      if (state.toastTimer) clearTimeout(state.toastTimer);
      state.toast = '';
    ...
    backendTableApplyPayload: function (state, payload) {
      ...
      var hasSchema = !!(payload.schema && Array.isArray(payload.schema.columns));
      var hasRows = Array.isArray(payload.rows) || Array.isArray(payload.data);
      var ackOnly = !!payload.mutationOnly && !hasSchema && !hasRows && !payload.pagination;
      if (ackOnly) {
        state.error = payload.ok === false ? (payload.message || payload.error || payload.detail || 'Table mutation failed') : state.error;
        return;
      }
      ...
      state.validationRules = clone((payload.validation || {}).fields || payload.validationFields || state.validationRules || {});
      ...
      var payloadRows = Array.isArray(payload.rows) ? payload.rows : (Array.isArray(payload.data) ? payload.data : null);
      state.columns = normalizeColumns(schemaColumns.length ? schemaColumns : state.columns);
      if (payloadRows !== null) state.rows = normalizeRows(payloadRows);
      ...
      state.error = payload && payload.ok === false ? (payload.message || payload.error || payload.detail || 'Table request failed') : state.error;
    ...
    backendTableShowPatientBanner: function (state) {
      var pr = (state || {}).patientRegistration || {};
      if (!pr || !Object.keys(pr).length) return false;
      if ((pr.notice || pr.statusMessage || pr.entryPoint || pr.duplicateCandidateCount) && String(pr.notice || pr.statusMessage || pr.entryPoint || '').indexOf('<') < 0) return true;
      if (Array.isArray(pr.workflow) && pr.workflow.length) return true;
      return this.backendTablePatientQueueList(state).length > 0;
    },
    ...
    backendTableCellEditContextMenuStyle: function (state) {
      var menu = ((state || {}).cellContextMenu || {});
      return { left: Math.max(6, Math.min(+menu.left || 6, (window.innerWidth || 1024) - 180)) + 'px', top: Math.max(6, Math.min(+menu.top || 6, (window.innerHeight || 768) - 130)) + 'px' };
    },
    backendTableOpenCellEditContextMenu: function (tableId, event) {
      var state = this.backendTableState(tableId);
      if (!event) return;
      if (event.preventDefault) event.preventDefault();
      if (event.stopPropagation) event.stopPropagation();
      state.cellContextMenu = { open: true, left: event.clientX || 0, top: event.clientY || 0 };
    },
    backendTableCloseCellEditContextMenu: function (tableId) {
      var state = this.backendTableState(tableId);
      state.cellContextMenu = { open: false, left: 0, top: 0 };
    },
    backendTableRunCellEditClipboard: function (tableId, action) {
      var active = document.activeElement;
      var state = this.backendTableState(tableId);
      function canText(el) { return el && (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA'); }
      if (!canText(active)) { state.cellContextMenu.open = false; return; }
      if (action === 'copy' || action === 'cut') {
        try { document.execCommand(action); } catch (err) {}
      } else if (action === 'paste') {
        if (navigator.clipboard && navigator.clipboard.readText) {
          navigator.clipboard.readText().then(function (text) {
            var start = active.selectionStart || 0;
            var end = active.selectionEnd || start;
            var next = String(active.value || '').slice(0, start) + text + String(active.value || '').slice(end);
            active.value = next;
            active.dispatchEvent(new Event('input', { bubbles: true }));
          }).catch(function () { try { document.execCommand('paste'); } catch (err) {} });
        } else { try { document.execCommand('paste'); } catch (err) {} }
      }
      state.cellContextMenu.open = false;
    },
        ...
        if (payload && payload.ok !== false) {
          ...
          if (dialog.context === 'editor' && state.editor && state.editor.row) {
            if (filterControlForColumn(column) === 'multiselect') {
              var parts = String(state.editor.row[column.key] || '').split(',').map(function (part) { return part.trim(); }).filter(Boolean);
              if (parts.indexOf(value) < 0) parts.push(value);
              state.editor.row[column.key] = parts.join(', ');
            } else state.editor.row[column.key] = value;
          }
          if (dialog.context === 'cell' && state.cellEditor) {
            if (filterControlForColumn(column) === 'multiselect') {
              var vals = String(state.cellEditor.value || '').split(',').map(function (part) { return part.trim(); }).filter(Boolean);
              if (vals.indexOf(value) < 0) vals.push(value);
              state.cellEditor.value = vals.join(', ');
            } else state.cellEditor.value = value;
          }
    ...
    backendTableValidationRule: function (state, key) {
      return (((state || {}).validationRules || {})[key]) || {};
    },
    backendTableColumnRequired: function (state, column) {
      var key = (column || {}).key || '';
      if (!key || key === 'id') return false;
      var rule = this.backendTableValidationRule(state, key);
      return !!(+((column || {}).required || 0) || +((column || {}).isRequired || 0) || +(rule.required || 0));
    },
    backendTableEditorColumns: function (state) {
      var vm = this;
      return normalizeColumns((state || {}).columns || []).filter(function (col) {
        if (!col || !col.key) return false;
        if (col.key === 'id') return false;
        if (col.editable === false || col.editable === 0 || col.editable === '0') return false;
        if (col.hidden && !vm.backendTableColumnRequired(state, col)) return false;
        return true;
      });
    },
    ...
    backendTableValidateEditor: function (state) {
      ...
      if (editor.mode === 'column') {
        ...
        var editable = this.backendTableEditorColumns(state);
        editable.forEach(function (col) {
          var rule = (((state || {}).validationRules || {})[col.key]) || {};
          var required = !!(+((col || {}).required || 0) || +((col || {}).isRequired || 0) || +(rule.required || 0));
          if (required && !String((editor.row || {})[col.key] || '').trim()) errors[col.key] = (rule.message || (col.label || col.key) + ' is required');
        });
        if (!Object.keys(errors).length && editable.length && !editable.some(function (col) { return String((editor.row || {})[col.key] || '').trim(); })) errors.row = 'At least one editable field must be provided.';
      }
      ...
      template: '' +
        '<section class="mioos-full-table" tabindex="0" @keydown.esc.stop="vm.backendTableHandleKeydown(tableId || state.id, $event)" :class="[\'density-\' + ((state.config || {}).density || \'compact\'), { \'is-processing\': state.loading || state.saving, \'is-readonly\': state.readOnly }]" :aria-busy="(state.loading || state.saving) ? \'true\' : \'false\'">' +
          '<header class="mioos-table-toolbar">' +
            ...
            '<button v-if="vm.backendTableFeature(state, &quot;rowCrud&quot;)" type="button" class="mioos-btn" @click="vm.backendTableOpenRowEditor(tableId || state.id)">Add row</button><button v-if="vm.backendTableShowPatientBanner(state)" type="button" class="mioos-btn" @click="vm.backendTableOpenPatientImport(tableId || state.id)">Import CSV</button><button v-if="vm.backendTableShowPatientBanner(state)" type="button" class="mioos-btn" @click="vm.backendTableRunPatientReconcile(tableId || state.id)">Reconcile</button>' +
