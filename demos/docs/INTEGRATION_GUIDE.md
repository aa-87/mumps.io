...
MIOTPL caches compiled tokens under `^MIO("TPL","CACHE",...)`. The cache is automatically used by `RENDER/GETTOKFP/GETTOKREF`.
...
N OREF,ERR
S OREF=$NA(^TMP($J,"OUT"))
D RENDERREF^MIOTPL("pages/big",.CONF,.CTX,OREF,.ERR)
```
...
N OUT,ERR,OPT
S OPT("mode")="AUTO"
D RENDERX^MIOTPL("pages/big",.CONF,.CTX,.OUT,.ERR,.OPT)
```
