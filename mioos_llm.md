...
## ROI 102 handoff — viewer regression repair

- Preserve Markdown/HTML preview-first behavior: `previewOnOpen` must keep `textViewerRenderDocumentPreview` from being skipped by `stream.editing` after chunk load.
- Do not reintroduce modified-time/size-only stale conflict checks. They caused false `stale_text_revision_conflict` failures because explorer entry metadata and `/api/mioos/fs/meta` formatting can differ. Only stable hashes/ETags/revisions should be used for future conflict checks.
- Normal text loading is explicit HTTP chunk loading with loop guards, non-advancing-offset guards, and a browser editor cap. Do not wire wheel/pointer/touch/key/scroll events to text chunk loading for normal viewers.
- Media viewer URLs should not force `stream=media`. Let the browser issue Range requests and keep `FSBLOB^MIOOSAPI` from sending synthetic initial 206 media windows unless explicitly configured.
