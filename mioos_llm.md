...
## ROI 101 handoff — dark tables and explicit editable text open/save

- Use only the current source tree. Preserve the MUMPS/YottaDB, MIOTPL, MIOOSAPI/MIOOSWS, MIOOSFS/MIOOSFSUP, Vue 3 Options API UMD architecture; no build step, TypeScript, Composition API, Node/Express, npm, or CDN dependency.
- Dark table readability is now variable-driven. Preserve `--mioos-table-*` variables and scoped `.theme-dark-mode` selectors for `mioos-full-table`, `mioos-surface-table`, Advanced Table filters/modals, Patient Registration, permissions grids, and module table previews. Do not reintroduce broad wildcard dark overrides such as `.theme-dark-mode *` for color/background/scrollbar handling.
- Text files open as explicit editable HTTP chunk sessions (`loadStrategy='explicit-complete-http'`) rather than scrollbar-dependent virtual loading. Scroll events should not trigger text chunk requests for normal text editing, and there must be no idle visible-range/prefetch loop.
- Text chunks remain clamped to the safe text chunk size (`65536` default). Chunk size and any threshold/telemetry value remain separate; do not raise WebSocket payload size or send large text over WebSocket JSON.
- Large text save checks fresh metadata for stale modified/size conflicts, then uses authenticated HTTP upload staging (`begin/chunk/commit`) with bounded chunks, abort on failure, committed-size verification when available, cache clearing, and reload/revalidation after success. `/api/mioos/fs/text-save` and `fs.text.save` are small compatibility paths and reject payloads above the safe full-JSON limit with `fs_text_save_requires_chunked`.
- Do not claim unlimited editing. The transfer/save path is chunk safe, but the browser editor still needs one full editable string after open; extremely large files are limited by practical browser/editor memory and should fail clearly instead of silently truncating or corrupting data.
