...
## ROI 70 review queues and duplicate resolution

Open the module directly from the shell as **Patient Registration** or through App Catalogue > Healthcare > Patient Registration. The direct entry is backed by `win-patient-registration` in `MIOOSST` and module id `mioos.ui.patient.registration`.

Useful MUMPS checks:

```mumps
D ENSURE^MIOOSTBL(.STATE,"patient-registration")
SET IN("dataset")="patient-registration",IN("filters","reviewQueue","mode")="include",IN("filters","reviewQueue","value")="Pending Review"
D QUERY^MIOOSTBL(.STATE,.CONF,.IN,.OUT,.ERR)
```

Mark a duplicate candidate:

```mumps
SET IN("dataset")="patient-registration",IN("action")="patient.duplicate.mark",IN("rowId")="PAT-1001"
D MUTATE^MIOOSTBL(.STATE,.CONF,.IN,.OUT,.ERR)
```

Review and duplicate actions are server-side in `MIOOSPAT`; the browser only sends the table mutation contract.
